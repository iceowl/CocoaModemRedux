...
@synthesize contestCallsign  = _contestCallsign;
@synthesize contestName      = _contestName;
@synthesize prototypeName    = _prototypeName;
...
- (Contest*)selectContest:(NSString*)newContestName parser:(NSXMLParser*)parser
{
	...
	master = [ self allocContest:_contestName ] ;
	if ( master ) {
		test = [ master initContestName:_contestName prototype:_prototypeName parser:parser manager:self ] ;
		if ( test ) {
			//[ test retain ] ;
...
- (void)addContestClient:(ContestInterface*)modem
{
	...
	if ( master ) {
		...
		clientContest = [ self allocContest:_contestName ] ;
...
- (void)updateQSOInfo
{
	...
	if ( qsoInfo ) {
		...
		if ( number ) {
			sscanf( [ number cStringUsingEncoding:NSASCIIStringEncoding  ], "%d", &num ) ;
		...
		if ( userInfo && call && number ) {
			...
			if ( !qth || [ qth length ] == 0 ) isDX = YES ;
				...
				qths = [ qth cStringUsingEncoding:NSASCIIStringEncoding  ] ;
...
- (void)saveMacrosToXML:(FILE*)file
{
	...
	fprintf( file, "\t<macros>\n" ) ;
	for ( i = 0; i < 6; i++ ) {
		fprintf( file, "\t\t<caption>%s</caption>\n", [ [ contestMacroSheet[i] captions ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
		fprintf( file, "\t\t<body>%s</body>\n", [ [ contestMacroSheet[i] messages ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	}
	fprintf( file, "\t</macros>\n" ) ;
}
...
- (void)contestWithPath:(NSString*)path
{
	...
	f = fopen( [ path cStringUsingEncoding:NSASCIIStringEncoding  ], "r" ) ;	
    if ( f == nil ) return ;
	fclose( f ) ;
...
//	[ xmlParser release ] ;
	...
	[ stdManager selectContest:_contestName parser:xmlParser ] ;
...
- (IBAction)newContest:(id)sender
{
	...
	test = fopen( [ path cStringUsingEncoding:NSASCIIStringEncoding  ], "r" ) ;
	if ( test ) {
		fclose( test ) ;
...
- (IBAction)continueContest:(id)sender
{
    ...
    [ open setAllowedFileTypes:[ NSArray arrayWithObjects:@"xml", @"jnl", nil ]];
	//result = [ open runModalForDirectory:nil file:nil types:[ NSArray arrayWithObjects:@"xml", @"jnl", nil ] ] ;
    result = (int)[open runModal];
	if ( result == NSOKButton ) {
		path = [ [ open URLs ] objectAtIndex:0 ] ;
		inputFile = fopen( [[ path absoluteString ] cStringUsingEncoding:NSASCIIStringEncoding], "r" ) ;
		...
		if ( inputFile && tempFile ) {
			...
			if ( isJournal ) {
				...
				[ preference setString:[path absoluteString] forKey:kRecentContest ] ;
				[ recentContestMenuItem setTitle:[path absoluteString] ] ;
			}
			fclose( tempFile ) ;
			fclose( inputFile ) ;
		}
		[ self contestWithPath:[ NSString stringWithCString:kTempFile encoding:NSASCIIStringEncoding] ] ;
		dirty = YES ;
	}
}
...
{
	if ( _saveFileName == nil ) {
		[ self saveContestAs:self ] ;
		return ;
	}
	[ master saveContest:_saveFileName ] ;
	dirty = NO ;
}
...
- (IBAction)saveContestAs:(id)sender
{
	...
	if ( _contestName == nil ) name = @"Contest" ; else name = _contestName ;
	fullname = [ name stringByAppendingFormat:@" %d.xml", year ] ; 
	NSURL *thisUrl = [[NSURL alloc]initWithString:fullname];
	save = [ NSSavePanel savePanel ] ;
    [save setDirectoryURL:thisUrl];
	result = (int)[ save runModal ] ;
	if ( result == NSOKButton ) {
		//if ( saveFileName ) [ saveFileName release ] ;
		_saveFileName = [ [ save URL ] absoluteString ];
		[ master saveContest:_saveFileName ] ;
		dirty = NO ;
	}
}
...
- (IBAction)createCabrillo:(id)sender
{
	...
	if ( master ) {
        ...
        result = (int)[save runModal];
		if ( result == NSOKButton ) {
			NSURL *upath = [ save URL ] ;
			[ master writeCabrilloToPath:[upath absoluteString] callsign:callsign ] ;
		}
	}
}
