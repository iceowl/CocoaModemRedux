...
@synthesize installedModem = _installedModem;
...
-(id) init {
    ...
    if(self){
        ...
        _installedModem = [[NSMutableArray alloc] initWithCapacity:MAXMODEM];
...
- (void)setupWindow:(Boolean)textured lite:(Boolean)lite
{
	...
	[ tabview setDelegate:(id <NSTabViewDelegate>)self ] ;
}
...
- (void)applicationTerminating
{
	...
	for ( i = 0; i < installedModems; i++ ) {
		[ ((InstalledModem*)[_installedModem objectAtIndex:i]).modem applicationTerminating ] ;
	}
}
...
- (void)switchToTabView:(int)index
{
	...
	currentIndex = (int)[ tabview indexOfTabViewItem:[ tabview selectedTabViewItem ] ] ;
...
- (Boolean)selectModem:(Modem*)modem
{
	...
	if ( item ) {
		...
		if ( index != NSNotFound ) {
			[ self switchToTabView:(int)index ] ;
			return YES ;
		}
	}
	return NO ;
...
- (void)selectTabView:(NSString*)viewName
{
	...
	n = (int)[ tabview numberOfTabViewItems ] ;
...
- (void)setModemAtTab:(NSTabViewItem*)tabViewItem
{
	...
	for ( i = 0; i < [_installedModem count]; i++ ) {
        installed = _installedModem[i] ;
...
- (void)updateModemNow:(InstalledModem*)installed
{
	...
	if ( installed.updatedFromPlist == NO ) {
        ...
        [ installed.modem updateFromPlist:_preferences ] ;
	}
}
...
//		Gestalt( gestaltSystemVersionMinor, &systemVersion ) ;
        ...
        //Gestalt( gestaltSystemVersionMinor, &systemVersion ) ;
        float sysVersion = [productVersion doubleValue];
        sysVersion -= 10.0;
        systemVersion = (int)sysVersion;
		if ( systemVersion > 4 ) {
			//  v0.96c
...
- (void)selectNextModem
{
	...
	i = (int)[ tabview indexOfTabViewItem:[ tabview selectedTabViewItem ] ] ;
	n = (int)[ tabview numberOfTabViewItems ] ;
