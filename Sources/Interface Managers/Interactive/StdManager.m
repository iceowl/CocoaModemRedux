...
@synthesize qsoTabview = _qsoTabview;
...
- (void)setupWindow:(Boolean)textured lite:(Boolean)lite
{
	...
	_tooltip = [ view addToolTipRect:[ view bounds ] owner:@"" userData:nil ] ;
	[ view setToolTip:@"" ] ;
	...
	//  QSO/ContestBar
	_qsoTabview = ( textured ) ? qsoTabviewTextured : qsoTabviewUntextured ;
	qso = [ [ QSO alloc ] initIntoTabView:_qsoTabview app:application ] ;
	contestBar = [ [ ContestBar alloc ] initIntoTabView:_qsoTabview app:application ] ;
...
- (void)createModems:(Preferences*)pref startModemsFromPlist:(Preferences*)modemList
{
    ...
    int ins = 0;
	//installed = [modemList objectAtIndex:ins] ;
    ...
    installed = [[InstalledModem alloc] init];
	...
	if ( [ modemString characterAtIndex:kAMModemOrder ] == '1' && isLiteWindow == NO ) {
       ...
       // NSLog(@"init AM Modem");
		...
		[self.installedModem insertObject:installed atIndex:ins++] ;
        installed = [[InstalledModem alloc] init];
	}
   //  NSLog(@"init FAX Modem");
  ...
  //       NSLog(@"init RTTY Modem");
		//installedModems = self.installedModem[ins+1] ;		//  need to update before calling initIntoTabView v0.41
...
- (void)updateModemSources
{
   ...
   InstalledModem *installed ;
    ...
    for ( i = 0; i < [self.installedModem count]; i++ ) {
        installed = [self.installedModem objectAtIndex:0] ;
...
- (void)enableModems:(Boolean)state
{
	...
	for ( i = 0; i < [self.installedModem count]; i++ ) {
        installed = [self.installedModem objectAtIndex:i];
...
- (void)updateQSOWindow
{
	...
	[ _qsoTabview selectTabViewItemAtIndex:index ] ;
}
...
- (void)loadRTTYMacros:(Preferences*)pref
{
	...
	for ( i = 0; i < 3; i++ ) {
		...
		if ( macroSheet ) {
            ...
            for ( j = 0; j < [self.installedModem count]; j++ ) {
                installed = [self.installedModem objectAtIndex:j];
                if(installed) {
...
//	[ contestManager updateFromPlist:pref ] ;  // THIS HAS TO BE FIXED!!!
	
	// v0.53b -- this has been moved to super -willSelectTabViewItem
	/*
     InstalledModem *installed = &self.installedModem[0] ;
     int i ;
