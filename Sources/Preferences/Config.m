...
- (id)initWithApp:(Application*)app
{
	...
	self = [ super init ] ;
	if ( self ) {
		if ( [[ NSBundle mainBundle] loadNibNamed:@"Config" owner:self topLevelObjects:nil ] ) {
			logScriptFileName = @"" ;
			pttScriptFolderName = @"" ;
...
- (Boolean)browseForField:(NSTextField*)textField into:(NSString*)fname isFolder:(Boolean)isFolder
{
	...
	[ panel setAllowedFileTypes:[ NSArray arrayWithObjects:@"scpt", nil ]];
	...
	//	[ *fname release ] ;
		...
		if ( isFolder ) {
			//  complete path
			[ textField setStringValue:[thisUrl absoluteString]] ;
		}
		else {
			//  filename only
			[ textField setStringValue:[ fname lastPathComponent ] ] ;
		}
		return YES ;
	}
	fname = [ [ NSString alloc ] initWithString:[ textField stringValue ] ] ;
	return NO ;
}
...
- (Boolean)updatePreferences
{
    ...
    [ stdManager setPreferences:self];
	...
	if ( [ self hasKey:kEnableNetAudio ] ) {
		if ( [ self intValueForKey:kEnableNetAudio ] != 0 ) ip = [NSString stringWithCString:[ application localHostIP] encoding:NSASCIIStringEncoding ];
	}
	...
	if ( [ self hasKey:kEnableNetAudio ] ) {
		if ( [ self intValueForKey:kEnableNetAudio ] != 0 ) {
			ip = [NSString stringWithCString:[ application localHostIP] encoding:NSASCIIStringEncoding ];
	...
	if ( s ) {
		...
		appearanceString = [ s cStringUsingEncoding:NSASCIIStringEncoding  ] ;
		count = (int)[ s length ] ;
		for ( i = 0; i < count; i++ ) {
	...
	if ( s ) {
		pskString = [ s cStringUsingEncoding:NSASCIIStringEncoding  ] ;
		count = (int)[ pskPrefs numberOfRows ] ;
		for ( i = 0; i < count; i++ ) {
			state = pskString[i] ;
			if ( state == 0 ) break ;
	...
	count = (int)[ modemPrefs numberOfRows ] ;
	...
	if ( s ) {
		modemString = [ s cStringUsingEncoding:NSASCIIStringEncoding  ] ;
	...
	logScriptFileName = [ self stringValueForKey:kQSOScript ];
...
//	[ array release ] ;
	...
	*s = 0 ;
	[ self setString:[ NSString stringWithCString:str encoding:NSASCIIStringEncoding ] forKey:kAppearancePrefs ] ;
	...
	*s = 0 ;
	[ self setString:[ NSString stringWithCString:str encoding:NSASCIIStringEncoding ] forKey:kPSKPrefs ] ;
	...
	for ( ; i < 15; i++ ) *s++ = '1' ;
	*s = 0 ;
	[ self setString:[ NSString stringWithCString:str encoding:NSASCIIStringEncoding ] forKey:kModemList ] ;
