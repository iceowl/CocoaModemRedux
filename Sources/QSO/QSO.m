...
- (NSString*)callsign
{
	...
	strippedCallsign = [self asciiString:string ]  ;
...
- (NSString*)opName
{
	...
	strippedOp = [ self asciiString:string ]  ;
	return strippedOp ;
}
...
- (id)initIntoTabView:(NSTabView*)tabview app:(Application*)app
{
	...
	if ( self ) {
		...
		if ( [ [NSBundle mainBundle] loadNibNamed:@"QSO" owner:self topLevelObjects:nil ] ) {
...
- (NSString*)getRegisteredTime
{
	...
	return [ NSString stringWithCString:format encoding:NSASCIIStringEncoding] ;
}
...
- (NSString*)cutFor:(NSString*)original
{
	...
	strcpy( c, [ original cStringUsingEncoding:NSASCIIStringEncoding ] ) ;
	for ( i = 0; i < 5; i++ ) {
		if ( c[i] == 0 ) break ;
...
- (NSString*)macroFor:(int)c
{
	...
	case 't':
		...
		str = [ NSString stringWithCString:format encoding:NSASCIIStringEncoding] ;
		break ;
	case 'T':
		sprintf( format, "%02d%02d", registeredTime.tm_hour, registeredTime.tm_min ) ;
		str = [ NSString stringWithCString:format encoding:NSASCIIStringEncoding] ;
...
- (NSString*)macroFor:(int)c count:(int)n
{
	...
	case 'n':
		//  number
		sscanf( [ qsoNumber cStringUsingEncoding:NSASCIIStringEncoding ], "%d", &p ) ;
		switch ( n ) {
		case 0:
			format = @"%d" ;
			break ;
		...
		str =  [ NSString stringWithFormat:format, p ] ;
		break ;
	case 'N':
		//  cut number
		sscanf( [ qsoNumber cStringUsingEncoding:NSASCIIStringEncoding], "%d", &p ) ;
	...
	case 'p':
		//  previous number
		sscanf( [ str cStringUsingEncoding:NSASCIIStringEncoding ], "%d", &p ) ;
...
- (void)copyString:(char*)selectedString into:(int)field
{
	...
	if ( f ) [ f setStringValue:[ NSString stringWithCString:selectedString encoding:NSASCIIStringEncoding ] ];
}
