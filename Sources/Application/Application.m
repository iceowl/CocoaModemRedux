...
NSThread     *mainThread ;
CoreModem    *thisunusedmodem;


@synthesize auralMonitor      = _auralMonitor;
@synthesize config            = _config;
@synthesize userInfo          = _userInfo;
@synthesize fskHub            = _fskHub;
@synthesize digitalInterfaces = _digitalInterfaces;
@synthesize macroScripts      = _macroScripts;
...
- (void)getLocalHostIP
{
	...
	while ( ( address = [ addresses nextObject ] ) != nil ) {
		ipAddr = [ address cStringUsingEncoding:NSASCIIStringEncoding] ;
		s = ipAddr ;
		while ( *s ) {
			if ( *s++ == '.' ) {
	...
	for ( i = 0; i < 4; i++ ) {
		...
		if ( service && [ service length ] > 0 ) {
			...
			if ( ( ip && [ ip length ] ) || ( portNum && [ portNum length ] ) ) {
				...
				if ( ip && [ ip length ] > 0 ) {
					ipAddr = [ ip cStringUsingEncoding:NSASCIIStringEncoding] ;
				}
				else {
	...
	if ( prefArray != nil ) {
		...
		for ( j = 0; j < count; j++ ) {
			...
			if ( str && [ str length ] > 0 ) {
				...
				if ( isInput ) {
					//  NetReceive
					sscanf( [ str cStringUsingEncoding:NSASCIIStringEncoding], "%d.%d.%d.%d:%d", &ip1, &ip2, &ip3, &ip4, &port ) ;
					if ( ip1 < 0 || ip2 < 0 || ip3 < 0 || ip4 < 0 || port < 0 ) {
						...
						strcpy( cstr, [ str cStringUsingEncoding:NSASCIIStringEncoding ] ) ;
						for ( i = 0; i < 64; i++ ) {
							if ( cstr[i] == ':' ) {
								cstr[i] = 0 ;
								break ;
					...
					str = [ NSString stringWithCString:cstr encoding:NSASCIIStringEncoding] ;
...
- (void)awakeFromNib
{
	...
	thisunusedmodem =[ [ CoreModem alloc ] init ] ;
	...
	if ( dontOpenRouter ) {
		_digitalInterfaces = [ [ DigitalInterfaces alloc ] initWithoutRouter ] ;
	}
	else {
		_digitalInterfaces = [ [ DigitalInterfaces alloc ] init ] ;
	}
	_macroScripts = [ [ MacroScripts alloc ] init ] ;				//  v0.89
	
	isBrushedMetal = isLite = NO ;
	
	NSString *prefString = [ tempPref stringValueForKey:kAppearancePrefs ] ;
	if ( prefString != nil ) {												// v0.42 Leopard returning nil cStringUsingEncoding:NSASCIIStringEncoding UsingEncoding:NSASCIIStringEncoding 
		str = [ prefString cStringUsingEncoding:NSASCIIStringEncoding] ;
		if ( str != nil ) {
	...
	_auralMonitor = [ [ AuralMonitor alloc ] init ] ;
	...
	_config = [ [ Config alloc ] initWithApp:self ] ;
	[ _config awakeFromApplication ] ;
	
	//  create the modems based on what is asked for in the Preference panel
	
	// 0.54 use config as prefs
	[ stdManager createModems:_config startModemsFromPlist:tempPref ] ;
	//[ tempPref release ] ;
	...
	if ( isLite ) {
		...
		if ( [ _config intValueForKey:kHideWindow ] == 0 ) {
	...
	if ( [ _config booleanValueForKey:kVoiceAssist ] ) {
...
{
	[ _config showPreferencePanel:self ] ;
}
...
- (void)setAppearancePrefs:(NSMatrix*)appearancePrefs
{
	...
	count = (int)[ appearancePrefs numberOfRows ] ;
	for ( i = 0; i < count; i++ ) {
...
{
	[ _auralMonitor showWindow ] ;
}

- (IBAction)showUserInfo:(id)sender
{
	[ _userInfo showSheet:[ stdManager windowObject ] ] ;
}
...
- (IBAction)selectInterfaceMode:(id)sender
{
	...
	[ self switchInterfaceMode:(int)[ sender tag ] ] ;
}
...
- (void)transferToQSOField:(int)t
{
	...
	if ( t != 0 ) {
		...
		if ( _selectedTextView ) {
			//  unselect the field
			[ _selectedTextView lockFocus ] ;
			range = [ _selectedTextView selectedRange ] ;
			range.length = 0 ;
			[ _selectedTextView setSelectedRange:range ] ;
			[ _selectedTextView unlockFocus ] ;
...
- (NSApplicationTerminateReply)terminate
{
	...
	if ( ![ stdManager okToQuit ] ) {
		reply = (int)[ [ NSAlert alertWithMessageText:NSLocalizedString( @"database not saved", nil ) defaultButton:NSLocalizedString( @"OK", nil ) alternateButton:nil otherButton:NSLocalizedString( @"Quit anyway", nil ) informativeTextWithFormat:NSLocalizedString( @"save contest", nil ) ] runModal ] ;
	...
	// v0.50
	if ( _fskHub ) {
		[ _fskHub closeFSKConnections ] ;
		_fskHub = nil ;
	}
	if ( _digitalInterfaces ) [ _digitalInterfaces terminate:_config ] ;

//	[ sleepManager release ] ;			// this should deallocate it
	
	[ _config setBoolean:[ self voiceAssist ] forKey:kVoiceAssist ] ;
	[ _config savePlist ] ;

	//  v0.78
	[ _auralMonitor unconditionalStop ] ;
...
{
	if ( _audioManager != nil ) [ _audioManager putCodecsToSleep ] ;
}

//  called from ModemSleepManager
- (void)wakeCodecsUp
{
	if ( _audioManager != nil ) [ _audioManager wakeCodecsUp ] ;
}
...
//			sscanf( [ version cStringUsingEncoding:NSASCIIStringEncoding], "%f", &current ) ;
//			
