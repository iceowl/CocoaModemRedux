...
- (void)drawMarkers
{
	...
	[ self drawMarker:p width:1 color:self.green ] ;
	p = (int)( fl/5.38 - 14 ) - 0.5 ;
	[ self drawMarker:p width:2 color:self.black ] ;
	[ self drawMarker:p width:1 color:self.green ] ;
	p = (int)( fc/5.38 - 14 ) ;
	[ self drawShortMarker:p width:5 color:self.green ] ;
}
...
- (void)importAndDisplayInMainThread:(CMPipe*)pipe
{
	...
	if ( [ self.drawLock tryLock ] ) {
		...
		if ( mux >= 4096 ) {
			mux = 0 ;
			CMPerformFFT( self.spectrum, timeSample, freqBin ) ;
