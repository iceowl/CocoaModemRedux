...
- (void)drawMarkers
{
	...
	if ( separateTxAndRx ) {
		rxColor = self.magenta ;
	}
	else {
		rxColor = self.green ;
		diff = 0 ;
	}	
	rxColor = ( separateTxAndRx ) ? self.magenta : self.green ;
	
	if ( !separateTxAndRx || diff > 2.0 ) {	
		p = mark[waterfallID]+0.5 ;
		[ self drawMarker:p width:2 color:self.black ] ;
	...
	if ( separateTxAndRx ) {
		p = txMark+0.5 ;
		[ self drawMarker:p width:2 color:self.black ] ;
		[ self drawMarker:p width:1.25 color:self.green ] ;
	}
	else {
		if ( fabs( ritOffset ) > 1.0 ) {
			p = mark[waterfallID]+ritOffset+0.5 ;
			[ self drawMarker:p width:2 color:self.black ] ;
			[ self drawMarker:p width:1.25 color:self.magenta ] ;
		}
	}
}
