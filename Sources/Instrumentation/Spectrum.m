...
@synthesize spectrumColor   = _spectrumColor;
@synthesize backgroundColor = _backgroundColor;
@synthesize markSpaceColor  = _markSpaceColor;
@synthesize path            = _path;
@synthesize background      = _background;
@synthesize spectrumScale   = _spectrumScale;
@synthesize markSpace       = _markSpace;
...
- (id)initWithFrame:(NSRect)frame 
{
    ...
    if ( self ) {
		...
		for ( i = 0; i < 512; i++ ) smoothedSpectrum[i] = 0 ;
		 ...
		 _path = nil ;
		_background = [ [ NSBezierPath alloc ] init ] ;
		[ _background appendBezierPathWithRect:bounds ] ;
		_backgroundColor = [ NSColor colorWithDeviceRed:0 green:0.1 blue:0 alpha:1 ];
		
		//  set up spectrum scale
		_spectrumColor = [ NSColor colorWithCalibratedRed:0.9 green:0.9 blue:0 alpha:1 ] ;
		_spectrumScale = [ [ NSBezierPath alloc ] init ] ;
		[_spectrumScale setLineDash:dash count:2 phase:0 ] ;
		for ( i = 5; i < 80; i += 30 ) {
			y = (int)( height - ( i*pixPerdB ) ) + 0.5 ;
			if ( y <= 0 ) break ;
			[_spectrumScale moveToPoint:NSMakePoint( 0, y ) ] ;
			[_spectrumScale lineToPoint:NSMakePoint( plotWidth, y ) ] ;
		}
		for ( i = 500; i < 3000; i += 500 ) {
			x = ( int )( ( i-400 )*plotWidth/2200.0 ) + 0.5 ;
			[_spectrumScale moveToPoint:NSMakePoint( x, 0 ) ] ;
			[_spectrumScale lineToPoint:NSMakePoint( x, height ) ] ;
		}
		//  set up scale color
		_scaleColor = [ NSColor colorWithCalibratedRed:0 green:1 blue:0.1 alpha:1 ] ;
				
		//  set up mark/space scales
		_markSpaceColor = [ NSColor colorWithCalibratedRed:0.88 green:0.15 blue:0 alpha:1 ] ;
		_markSpace = [ [ NSBezierPath alloc ] init ] ;
		//[_markSpacesetLineDash:closedash count:2 phase:0 ] ;
		x = ( int )( ( 2125-400 )*plotWidth/2200.0 ) + 0.5 ;
		[_markSpace moveToPoint:NSMakePoint( x, 8 ) ] ;
		[_markSpace lineToPoint:NSMakePoint( x, height-10 ) ] ;
...
{
	if ( _thread == [ NSThread currentThread ] && [ self lockFocusIfCanDraw ] ) {
		//  clear background
		[ _backgroundColor set ] ;
		[ _background fill ] ;
		//  insert scale
		[ _scaleColor set ] ;
		[_spectrumScale stroke ] ;
		// mark/space scale
		[ _markSpaceColor set ] ;
		[ _markSpace stroke ] ;
		//  insert graph
	...
	if ( mux >= 2048 ) {
		...
		for ( i = 0; i < 408; i++ ) {
			...
			if ( pen == YES ) {
				...
				if ( y < 0 || y >= height ) {
					if ( y < 0 ) {
						[  _path lineToPoint:NSMakePoint( x, 0.0 ) ] ; 
						ySat = 0.0 ; 
					}
					else {
						[  _path lineToPoint:NSMakePoint( x, height-1 ) ] ;
						ySat = height-1 ;
					}
					pen = NO ;
				}
				else [  _path lineToPoint:NSMakePoint( x, y ) ] ;
			}
			else {
				//  pen was in up state
				if ( y >= 0 && y < height ) {
					if ( i == 0 ) [  _path moveToPoint:NSMakePoint( x, y ) ] ;
					else {
						[  _path moveToPoint:NSMakePoint( x-1, ySat ) ] ;
...
- (void)setTonePairMarker:(const CMTonePair*)tonepair
{
	...
	_markSpace = [ [ NSBezierPath alloc ] init ] ;
	[_markSpace setLineWidth:0.5 ] ;
	x = m + 0.5 ;
	[_markSpace moveToPoint:NSMakePoint( x, 10 ) ] ;
	[_markSpace lineToPoint:NSMakePoint( x, height-10 ) ] ;
	x = s + 0.5 ;
	[_markSpace moveToPoint:NSMakePoint( x, 10 ) ] ;
	[_markSpace lineToPoint:NSMakePoint( x, height-10 ) ] ;
