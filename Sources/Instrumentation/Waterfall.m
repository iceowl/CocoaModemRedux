...
- (void)awakeFromModem
{
	...
	_black = [ NSColor blackColor ];
	_red = [ NSColor greenColor ];
	_magenta = [ NSColor magentaColor ] ;
...
- (void)drawMarkers
{
	...
	if ( click > 0 ) {
		p = click+0.5 ;
		[ self drawMarker:p width:2 color:_black ] ;
		[ self drawMarker:p width:1.25 color:_green ] ;
	}
	if ( optionClick > 0 ) {
		p = optionClick+0.5 ;
		[ self drawMarker:p width:2 color:_black ] ;
		[ self drawMarker:p width:1.25 color:_magenta ] ;
	}
}
...

{
    ...
    __block Waterfall* bSelf = self; // capture so as to not have compiler complain about ARC retain cycles...dunno if this is going to be a problem.
    
    if(!_timerQueue)  _timerQueue = dispatch_queue_create("com.owlhousetoys.waterfallTimer", DISPATCH_QUEUE_SERIAL);
    if((!timer1) && _timerQueue) {
       ...
       // g_timerQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        timer1 = [self  CreateDispatchTimer :100ull * NSEC_PER_MSEC
                                              ...
                                              :gBlock = ^{
                                                  if(*w_okToDraw){
                                                      [bSelf importAndDisplayData:w_wPipe];
                                                      *w_okToDraw = FALSE;
                                                  }
                                                  // usleep(10);
                                              }];
    ...
    else if((!timer2) && _timerQueue) {
        timer2 = [self  CreateDispatchTimer :100ull * NSEC_PER_MSEC
                                              :10ull * NSEC_PER_MSEC
                                              :_timerQueue
                                              :gBlock2 = ^{
                                                  if(*w_okToDraw){
        ...
        [gBlock2 copy]; // copy to heap so not sitting on stack with all those self vars...
    }
...
-(void)killWaterfallTimer {
    ...
    if(timer2) dispatch_source_cancel(timer2);
...
- (void)processBufferAndDisplayInMainThread:(float*)samples
{
    ...
    CMPerformFFT( _spectrum, samples, freqBin ) ;
    ...
    [ _image addRepresentation:_bitmap ] ;
    pixel = ( UInt32* )[ _bitmap bitmapData ] ;	//  this should retrieve the local buffer pointer
