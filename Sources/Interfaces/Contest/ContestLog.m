...
- (id)initWithManager:(ContestManager*)control
{
	...
	if ( self ) {
		...
		success = [ [NSBundle mainBundle] loadNibNamed:@"ContestLog" owner:self topLevelObjects:nil ] ;
...
- (void)awakeFromManager
{
	...
	[ tableView setDataSource:(id <NSComboBoxDataSource>)self ] ;
	[ tableView setDelegate:self ] ;		//  delegate sorting
	
	n = (int)[ (NSTableView*)tableView numberOfColumns ] ;
	...
	for ( i = 0; i < n; i++ ) {
		...
		[ column setIdentifier:[NSString stringWithFormat:@"%d",i] ] ;
...
{
	[ infoField setStringValue:[ NSString stringWithCString:info encoding:NSASCIIStringEncoding ] ] ;
}
...
- (void)setupDefaultPreferences:(Preferences*)pref
{
	...
	string[16] = 0 ;
	[ pref setString:[ NSString stringWithCString:string encoding:NSASCIIStringEncoding ] forKey:kContestLogOrder ] ;
	...
	[ pref setString:[NSString stringWithCString:string encoding:NSASCIIStringEncoding ] forKey:kContestLogSize ] ;
}
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	columnString = [ [ pref stringValueForKey:kContestLogOrder ] cStringUsingEncoding:NSASCIIStringEncoding  ] ;
	...
	widthString = [ [ pref stringValueForKey:kContestLogSize ] cStringUsingEncoding:NSASCIIStringEncoding  ] ;
...
- (int)sortQSOList:(ContestQSOObj*)p with:(SEL)method
{
	...
	//  sort by callsign
	lower = 0 ;
	upper = ( count = (int)[ qsoArray count ] ) - 1 ;
...
- (void)tableView:(NSTableView*)tableView setObjectValue:(id)object forTableColumn:(NSTableColumn*)tableColumn row:(int)index 
{
	...
	case 1:
		...
		strncpy( localString, [ is cStringUsingEncoding:NSASCIIStringEncoding  ], 63 ) ;
		localString[63] = 0 ;
	...
	case 3:
		...
		sscanf( [ is cStringUsingEncoding:NSASCIIStringEncoding  ], "%d", &metres ) ;
		if ( metres != old ) {
	...
	case 6:
		...
		strncpy( localString, [ is cStringUsingEncoding:NSASCIIStringEncoding  ], 3 ) ;
		localString[2] = 0 ;
		convertToUpper( localString ) ;
		if ( strcmp( localString, [ [ qso mode ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) == 0 ) return ;
