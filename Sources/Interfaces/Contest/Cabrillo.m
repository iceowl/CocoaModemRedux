...
@synthesize controllingWindow = _controllingWindow;
...
- (id)initWithManager:(ContestManager*)inManager
{
	...
	if ( self ) {
		...
		journalFile = nil ;
		_name = nil ;
		if ( [ [NSBundle mainBundle] loadNibNamed:@"Cabrillo" owner:self topLevelObjects:nil ] ) {
			rect = [ view bounds ] ;
...
- (void)showSheet:(NSWindow*)window
{
	...
	[ NSApp endSheet:self ] ;
	[ self orderOut:_controllingWindow ] ;
}
...
{
	fprintf( file, "\t\t<category>%s</category>\n", [ [ categoryMenu titleOfSelectedItem ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<band>%s</band>\n", [ [ bandMenu titleOfSelectedItem ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<cname>%s</cname>\n", [ [ nameField  stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<addr1>%s</addr1>\n", [ [ addr1Field stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<addr2>%s</addr2>\n", [ [ addr2Field stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<addr3>%s</addr3>\n", [ [ addr3Field stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<email>%s</email>\n", [ [ emailField stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<callused>%s</callused>\n", [ [ callUsedField stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<nameused>%s</nameused>\n", [ [ nameUsedField stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<operators>%s</operators>\n", [ [ operatorsField stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<club>%s</club>\n", [ [ clubField stringValue ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	fprintf( file, "\t\t<soapbox>%s</soapbox>\n", [ [ [ soapboxView textStorage ] string ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
}
...
- (IBAction)import:(id)sender
{
	...
	if ( result == NSOKButton ) {
		...
		prefs = [ [ Preferences alloc ] initWithPath:[path absoluteString]] ;
		if ( prefs ) {
			[ prefs fetchPlist:NO ] ;
...
- (IBAction)export:(id)sender
{
    ...
    NSURL *path = [[NSURL alloc] initWithString:@"ContestInfo.xml"];
    [save setDirectoryURL:path];
    result = (int)[save runModal];
	//result = [ save runModalForDirectory:nil file:@"ContestInfo.xml" ] ;
	if ( result == NSOKButton ) {
		prefs = [ [ Preferences alloc ] initWithPath:[[ save URL ] absoluteString] ] ;
		if ( prefs ) {
...
- (IBAction)selectTempFolder:(id)sender
{
    ...
    [ panel setAllowedFileTypes:[ NSArray arrayWithObjects:@"onlyPickFolders", nil ]];
	...
	if ( result == NSOKButton ) {
		current = [ [ panel URL ]  absoluteString];
...
- (FILE*)openJournalFile:(Contest*)contest
{
	...
	_name = [ folder stringByAppendingString:@"/cocoaModem " ] ;
	date = [ [ NSDate date ] descriptionWithCalendarFormat:@"%Y-%m-%d %H%M.jnl" timeZone:nil locale:nil ] ;
	_name = [ _name stringByAppendingString:date ] ;
	
	journalFile = fopen( [ _name cStringUsingEncoding:NSASCIIStringEncoding  ], "w" ) ;
