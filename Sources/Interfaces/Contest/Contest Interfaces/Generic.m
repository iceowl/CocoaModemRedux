...
- (void)newCallsign:(NSNotification*)notify
{
	...
	src = [ notify object ] ;	
	//capturedString = [ self asciicStringUsingEncoding:NSASCIIStringEncoding :[ (Modem*)src capturedString ] ] ;
...
- (void)newExchange:(NSNotification*)notify
{
	...
	src = [ notify object ] ;	
	//capturedString = [ self asciicStringUsingEncoding:NSASCIIStringEncoding :[ src capturedString ] ] ;
    capturedString = [self asciicStringUsingEncoding:[src capturedString]];
	length = (int)[ capturedString length ] ;
...
- (void)enterQSOFromXML
{
	...
	sscanf( [ qsoStrings[kQSOFreq] cStringUsingEncoding:NSASCIIStringEncoding  ], "%f", &p->frequency ) ;
...
- (void)writeCabrilloQSOs
{
	...
	myCall[0] = 0 ;
	if ( self.usedCallString ) {
		strncpy( myCall, [ self.usedCallString cStringUsingEncoding:NSASCIIStringEncoding  ], 16 ) ;
		myCall[13] = 0 ;
	}
	
	expanded = [ manager expandMacroInUserAndQSOInfo:[ [ cabrillo exchangeString ] cStringUsingEncoding:NSASCIIStringEncoding  ] ] ;
	strncpy( exchSent, [ expanded cStringUsingEncoding:NSASCIIStringEncoding  ], 16 ) ;
