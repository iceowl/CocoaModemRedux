...
- (Boolean)validateNumber:(NSString*)exchange
{
	...
	s = t = [ exchange cStringUsingEncoding:NSASCIIStringEncoding  ] ;
	c = *t++ & 0xff ;
...
- (ContestQSO*)createQSOFromCurrentData
{
	...
	length = (int)[ str1 length ]+(int)[ str2 length ]+2 ;
	p->exchange = ( char* )malloc( length ) ;
	sprintf( p->exchange, "%s-%s", [ str1 cStringUsingEncoding:NSASCIIStringEncoding  ], [ str2 cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
...
- (void)writeCabrilloFields
{
	...
	fprintf( cabrilloFile, "ARRL-SECTION: %s\n", [ section cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
}
