...
@synthesize savedCallsign   = _savedCallsign;
@synthesize savedExchange   = _savedExchange;
@synthesize usedCallString  = _usedCallString;
@synthesize previousCall    = _previousCall;
...
- (id)init
{
	...
	if ( self ) {
		...
		busy = NO ;
		_savedCallsign = _savedExchange = @"" ;
		_previousCall = [ [ NSMutableString alloc ] initWithCapacity:33 ] ;
		[ _previousCall setString:@"" ] ;
...
- (id)initIntoBox:(NSBox*)box contestName:(NSString*)name prototype:(NSString*)prototype modem:(ContestInterface*)inClient master:(Contest*)inMaster manager:(ContestManager*)mgr
{
	...
	if ( self ) {
		...
		_contestName = [ [ NSString alloc ] initWithString:name ] ;
		if ( box ) {
			//  subordinate
			if ( [ [NSBundle mainBundle] loadNibNamed:prototype owner:self topLevelObjects:nil] ) {
...
- (Callsign*)receivedCallsign:(NSString*)call band:(int)band isDupe:(Boolean*)isDupe
{
	...
	activeBand = band ;
	c = [ self getCallsignCheckingDupe:[ call cStringUsingEncoding:NSASCIIStringEncoding  ] ] ;
	if ( c == nil ) {
		*isDupe = YES ;
		if ( [ manager allowDupe ] ) {
			c = [ self getCallsign:[ call cStringUsingEncoding:NSASCIIStringEncoding  ] ] ;
		}
	}
	else *isDupe = NO ;
	dupeState = ( c == nil ) ;
...
- (void)saveXMLHead:(FILE*)file isJournal:(Boolean)isJournal
{
	...
	fprintf( file, "\t<contestName>%s</contestName>\n", [ _contestName cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	exchangeString = [ cabrillo exchangeString ] ;
	fprintf( file, "\t<sent>%s</sent>\n", [ exchangeString cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
...
- (void)saveXML:(NSString*)path
{
	...
	saveFile = fopen( [ path cStringUsingEncoding:NSASCIIStringEncoding  ], "w" ) ;
...
- (void)bandMenuChanged
{
	if ( self != activeSubordinate && master != nil ) {
		[ master switchBandTo:(int)[ [ bandMenu selectedItem ] tag ] index:(int)[ bandMenu indexOfSelectedItem ]  ] ;
	}
}
...
- (void)writeCabrilloCategory
{
	...
	//  category
	strcpy( category, [ [ cabrillo category ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	convertToUpper( category ) ;
	if ( strncmp( category, "SINGLE-OP", 9 ) == 0 ) {
		//  single op
		strcpy( band, [ [ cabrillo band ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
		convertToUpper( band ) ;
		convertToBand( band ) ;
...
- (void)writeCabrilloFields
{
	...
	while ( [ str length ] > 0 ) {
		if ( [ str length ] <= 64 ) {
			strcpy( kind, [ str cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
			str = @"" ;
		}
		else {
			strcpy( kind, [ [ str substringToIndex:64 ] cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
			for ( i = 63; i > 0 ; i-- ) {
				if ( kind[i] == ' ' || kind[i] == '\t' ) {
					kind[i] = 0 ;
					break ;
...
- (void)writeCabrilloToPath:(NSString*)path callsign:(NSString*)callsign
{
	...
	cabrilloFile = fopen( [ path cStringUsingEncoding:NSASCIIStringEncoding  ], "w" ) ;
	fprintf( cabrilloFile, "START-OF-LOG: 2.0\n" ) ;
	
	//  create a CREATED-BY field with version number
	version = [ [ NSBundle mainBundle ] objectForInfoDictionaryKey:@"CFBundleShortVersionString" ] ;
	if ( version ) {
		fprintf( cabrilloFile, "CREATED-BY: cocoaModem 2.0 v%s (W7AY)\n", [ version cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	}
	_usedCallString = callsign ;
	fprintf( cabrilloFile, "CALLSIGN: %s\n", [ callsign cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
