...
@synthesize workedColor = _workedColor;
- (void)awakeFromNib
{
	
	_workedColor = [ NSColor colorWithCalibratedRed:0 green:0.7 blue:0 alpha:0.9 ]  ;
}
...
- (void)updateCallArea:(int)area statelist:(StateList*)rawStateList
{
	...
	[ self workedAllArea:area color:_workedColor ] ;
}
	...
	for ( i = 0; i < 64; i++ ) {
		...
		if ( strcmp( rawStateList[i].abbrev, p->exchange ) == 0 ) {
			...
			if ( rawStateList[i].worked == 1 ) {
				[ self colorRow:rawStateList[i].y column:rawStateList[i].x color:_workedColor string:nil ] ;
...
- (void)showWindow:(StateList*)rawStateList
{
	...
	for ( i = 0; i < 64; i++ ) {
		if ( *rawStateList[i].abbrev == '*' ) break ;
		[ self colorRow:rawStateList[i].y column:rawStateList[i].x color:( rawStateList[i].worked )?_workedColor:[ NSColor blackColor ] string:[ NSString stringWithCString:rawStateList[i].abbrev encoding:NSASCIIStringEncoding] ] ;
	}
