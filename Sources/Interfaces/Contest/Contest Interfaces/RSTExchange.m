...
- (void)newCallsign:(NSNotification*)notify
{
	...
	//	capturedString = [ self asciicStringUsingEncoding:NSASCIIStringEncoding :[ src capturedString ] ] ;   // strip phi
        capturedString = [self asciicStringUsingEncoding:[src capturedString]];
...
- (void)newExchange:(NSNotification*)notify
{
	...
	if ( master ) {
		src = [ notify object ] ;	
		capturedString = [ self asciicStringUsingEncoding : [ src capturedString ] ] ;
		length = (int)[ capturedString length ] ;
...
- (void)setupFields
{
    ...
    [self setSavedCallsign:@""];
    [self setSavedExchange:@""];
...
- (void)enterQSOFromXML
{
	...
	t1 = t2 = t3 = 0 ;
	sscanf( [ qsoStrings[kQSOTime] cStringUsingEncoding:NSASCIIStringEncoding  ], "%d:%d:%d", &t1, &t2, &t3 ) ;
	t.hour = t1 ;
	t.minute = t2 ;
	t.second = t3 ;
	t1 = t2 = 1 ; 
	t3 = 5 ;
	sscanf( [ qsoStrings[kQSODate] cStringUsingEncoding:NSASCIIStringEncoding  ], "%d/%d/%d", &t1, &t2, &t3 ) ;
	t.day = t1 ;
	t.month = t2 ;
	t.year = t3 ;
	if ( [ qsoStrings[kQSOCall] length ] <= 0 ) {
		...
		activeCall = [ self hash:(char*)[ qsoStrings[kQSOCall] cStringUsingEncoding:NSASCIIStringEncoding  ] ] ;
	}
	p = (ContestQSO*)malloc( sizeof( ContestQSO ) ) ;
	p->frequency = 14.080 ;
	if ( [ qsoStrings[kQSOFreq] length ] > 0 ) sscanf( [ qsoStrings[kQSOFreq] cStringUsingEncoding:NSASCIIStringEncoding  ], "%f", &p->frequency ) ;
	importedFrequency = p->frequency ;
	
	//  QSO number
	if ( [ qsoStrings[kQSONumber] length ] > 0 ) {
		t1 = 0 ;
		sscanf( [ qsoStrings[kQSONumber] cStringUsingEncoding:NSASCIIStringEncoding  ], "%d", &t1 ) ;
		p->qsoNumber = t1 ;
	}
	...
	//  RST
	t1 = 599 ;
	if ( [ qsoStrings[kQSORST] length ] > 0 ) sscanf( [ qsoStrings[kQSORST] cStringUsingEncoding:NSASCIIStringEncoding  ], "%d", &t1 ) ;
	p->rst = t1 ;
	...
	if ( str ) {
		...
		strcpy( s, [ str cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
		p->exchange = s ;
	}
...
- (void)callFieldChanged
{
	...
	if ( [ string isEqualToString:self.previousCall ] ) return ;
	...
	[ self.previousCall setString:string ] ;
}
...
- (ContestQSO*)createQSOFromCurrentData
{
	...
	strcpy( p->exchange, [ str cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
...
- (void)logButtonPushed
{
	...
	if ( master ) {	
		//  save fields
	//	[ self.savedCallsign release ] ;
		[self setSavedCallsign: [ [ NSString alloc ] initWithString:[ dxCall stringValue ] ] ];
		//[ savedExchange release ] ;
		[self setSavedExchange:[ [ NSString alloc ] initWithString:[ dxExchange stringValue ] ] ];
		
		if ( [ (RSTExchange*)master validateExchange:self.savedExchange ] ) {
			//  get call from dxCall field if it was a dupe, create one
			if ( activeCall == nil ) {
				c = [ dxCall stringValue ] ;
				activeCall = [ self getCallsign:[ c cStringUsingEncoding:NSASCIIStringEncoding  ] ] ;
			}
...
- (void)parser:(NSXMLParser*)parser didStartElement:(NSString*)elementName namespaceURI:(NSString*)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	...
	printf( "cocoaModem read xml discarding element name %s\n", [ elementName cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
}
...
- (void)parser:(NSXMLParser*)parser foundCharacters:(NSString*)string 
{
	if ( parseContest ) {
		if ( parseContestLog ) {
			...
			if ( parseQSO ) {
				if ( parseQSOPhase ) {
					qsoStrings[parseQSOPhase] =  string ;
				}
			}
		}
		if ( parseContestName ) {
		//	if ( contestName ) [ contestName release ] ;
			[self setContestName: string ] ;
		}
	}
}
