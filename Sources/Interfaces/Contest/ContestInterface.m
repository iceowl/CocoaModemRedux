...
- (void)initContest:(Contest*)newContest master:(Contest*)inMaster manager:(ContestManager*)inManager
{
	...
	selectedField = 0 ;
	contest = [ contest initIntoBox:contestView contestName:[ master contestName ] prototype:[ master prototypeName ] modem:self master:inMaster manager:contestManager ] ;
	//if ( oldContest ) [ oldContest release ] ;
	[ self setClock:[ [ self.manager appObject ] clock ] ] ;
...
- (IBAction)transmitContestMessage:(id)sender
{
	...
	index = (int)[ sender selectedColumn ] ;
...
- (IBAction)contestModeChanged:(id)sender
{
	...
	if ( contestBar ) [ contestBar cancel ] ;
	contestModeIndex = (int)[ [ sender selectedCell ] tag ] ;
