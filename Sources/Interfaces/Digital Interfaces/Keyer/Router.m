...
- (int)findPorts:(NSString**)path stream:(NSString**)stream max:(int)maxCount
{
	...
	while ( ( modemService = IOIteratorNext( serialPortIterator ) ) && count < maxCount ) {
        ...
        if ( cfString ) {
			stream[count] = [  NSString stringWithString:(__bridge NSString*) cfString ]  ;
     //       CFRelease( cfString ) ;
			cfString = IORegistryEntryCreateCFProperty( modemService, CFSTR(kIOCalloutDeviceKey), kCFAllocatorDefault, 0 ) ;
			if ( cfString )  {
				path[count] = [  NSString stringWithString:(__bridge NSString*) cfString ] ;
			//	CFRelease( cfString ) ;
				count++ ;
			}
		}
	...
	if ( routerRd > 0 && routerWr > 0 ) {
		...
		if ( version > 1.79 ) {
			...
			for ( i = 0; i < 16; i++ ) {
				...
				count = (int)read( routerRd, response, 20 ) ;
				if ( response[0] == 0 ) break ;
				keyer = [ [ MicroKeyer alloc ] initWithKeyerID:response ] ;
				if ( keyer != nil ) [ _keyers addObject: keyer ] ;									//  v0.92 sanity check
			}
		}
		else {
...
//	name = [ NSString stringWithCString:cStringUsingEncoding:NSASCIIStringEncoding  encoding:NSMacOSRomanStringEncoding ] ;
...
//			strcpy( cStringUsingEncoding:NSASCIIStringEncoding , [ name cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
//			cStringUsingEncoding:NSASCIIStringEncoding [10] = 'M' ;	//  change usbserial-DK and usbserial-CK to MK
//			if ( strcmp( cStringUsingEncoding:NSASCIIStringEncoding , "usbserial-MK" ) == 0 || strcmp( cStringUsingEncoding:NSASCIIStringEncoding , "usbserial-M2" ) == 0 ) hasKeyer = YES ;
//			[ stream[i] release ] ;
//			[ path[i] release ] ;
//		}
//		if ( hasKeyer ) {
...
//							if ( response ) {
//								sscanf( [ response cStringUsingEncoding:NSASCIIStringEncoding  ],"%f", &version ) ;
//							}
//						}
//					}
...
- (id)init
{
	...
	if ( self ) {
		launched = NO ;
		version = 1.7 ;
		_keyers = [ [ NSMutableArray alloc ] init ] ;
...
- (NSAppleScript*)loadScriptForPath:(NSString*)path withErrorDictionary:(NSDictionary**)dict
{
	...
	if ( [ path length ] > 0 ) {
		...
		if ( !script ) {
			[ Messages appleScriptError:*dict script:(const char*)[ path cStringUsingEncoding:NSASCIIStringEncoding  ] ] ;
			return nil ;
		}
	}
	return script ;
