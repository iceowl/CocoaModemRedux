...
- (void)pollThread
{
	...
	__block int fd, i, count, bytes ;
    
    if(_FSKHubQueue == nil) _FSKHubQueue = dispatch_queue_create("com.owlhousetoys.FSKHubQueue", DISPATCH_QUEUE_CONCURRENT);
    dispatch_async(_FSKHubQueue, [ ^(void) {
        @autoreleasepool {
                    ...
                    for ( i = 0; i < activeKeyers; i++ ) {
                        ...
                        if ( fd > 0 ) {
                            if ( FD_ISSET( fd, &readSet ) ) {
                                bytes =(int) read( fd, tempBuffer, 1 ) ;
