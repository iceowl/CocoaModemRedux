...
- (id)initIntoView:(NSView*)view client:(Modem*)modem
{
	...
	if ( self ) {
		...
		psk = (PSK*)modem ;
		index = 0 ;
		if ( [ [NSBundle mainBundle] loadNibNamed:@"PSKTransmitControl" owner:self topLevelObjects:nil] ) {
			// loadNib should have set up controlView connection
			if ( view && controlView ) [ view addSubview:controlView ] ;
			index = (int)[ [ vfoMenu selectedItem ] tag ] ;
			return self ;
		}
	}
	return nil ;
...
- (int)selectedTransceiver
{
	...
	n = (int)[ [ vfoMenu selectedItem ] tag ] ;
	if ( psk ) {
		p = [ psk auralMonitor ] ;
...
- (void)selectTransceiver:(int)new
{
	...
	old = (int)[ [ vfoMenu selectedItem ] tag ] ;
	if ( old == new ) return ;
	
	n = (int)[ vfoMenu numberOfItems ] ;
	for ( i = 0; i < n; i++ ) {
...
- (void)vfoMenuChanged
{
	...
	if ( psk ) {
		newIndex = (int)[ vfoMenu indexOfSelectedItem ] ;
		if ( [ psk transmitting ] ) {
