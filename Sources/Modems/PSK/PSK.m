...
@synthesize transmitViewLock    = _transmitViewLock;
@synthesize transmitBufferCheck = _transmitBufferCheck;
@synthesize auralAction         = _auralAction;
@synthesize subDictionary       = _subDictionary;
...
- (id)initIntoTabView:(NSTabView*)tabview manager:(ModemManager*)mgr
{
	...
	if ( self ) {
        _auralAction   = [[NSMutableArray alloc] initWithCapacity:MAXAURALACTION];
        _subDictionary = [[NSMutableArray alloc] initWithCapacity:MAXAURALACTION];
		self.manager = mgr ;
		transceivers = 2 ;		
		//  v0.78
        ...
        for(int w = 0; w < MAXAURALACTION; w++) {
            _auralAction[w]   = [[AuralAction alloc]init];
            _subDictionary[w] = [[SubDictionary alloc] init];
        }
	}
	return self ;
}
...
{
	[ self.manager setAllowShiftJISForPSK:state ] ;
}
...
- (void)awakeFromNib
{
	...
	[self setIdent:NSLocalizedString( @"PSK", nil ) ];
	...
	[ self.modemTabItem setLabel:self.ident ] ;
	...
	transmitCount = 0 ;
	self.transmitCountLock = [ [ NSLock alloc ] init ] ;
	_transmitViewLock = [ [ NSLock alloc ] init ] ;
...
{
	[ self.manager showSplash:@"Updating PSK sound source" ] ;
...
- (void)enableChanged:(NSButton*)sender
{
	int i ;
	...
	for ( i = 0; i < 4; i++ ) {
        AuralAction *aa = [_auralAction objectAtIndex:i];
		if ( aa.enableButton == sender ) {
...
- (void)floatMatrixChanged:(id)sender
{
	...
	for ( i = 0; i < 3; i++ ) {
        AuralAction *aa = [_auralAction objectAtIndex:i];
		if ( aa.floatMatrix == sender ) {
			n = (int)[ sender selectedRow ] ;
...
{
	if ( ![ self.manager modemIsVisible:self ] ) return ;
...
- (void)setupAuralMonitorDefaultPreferences:(Preferences*)pref
{
	int i ;
	SubDictionary *p ;
	if(_auralMonitorPlist == nil) _auralMonitorPlist = [[NSMutableArray alloc] initWithCapacity:MAXAURALACTION];
	...
	for ( i = 0; i < 4; i++ ) {
		_auralMonitorPlist[i] = [ [ SubDictionary alloc ] init ] ;
	}
	
	//  initially disable all channels
	for ( i = 0; i < 4; i++ ){
        SubDictionary *sd = [_auralMonitorPlist objectAtIndex:i];
        [sd setInt:0 forKey:kMonitorEnable ] ;
    }
	
	for ( i = 0; i < 2; i++ ) {
		//  receiver defaults
		p = _auralMonitorPlist[i] ;
	...
	p = _auralMonitorPlist[2] ;
	[ p setFloat:6.0 forKey:kMonitorAttenuator ] ;
	[ p setInt:0 forKey:kMonitorFixSelect ] ;
	[ p setFloat:1048.0 forKey:kMonitorFrequency ] ;
	
	//  wideband default
    SubDictionary *sd = [_auralMonitorPlist objectAtIndex:3];
	[ sd setFloat:10.0 forKey:kMonitorAttenuator ] ;
}
...
- (void)updateFromAuralMonitorPlist:(Preferences*)pref
{
	...
	if ( d != nil ) [ [ [_auralMonitorPlist objectAtIndex:0] dictionary ] addEntriesFromDictionary:d ] ;
	
	d = [ pref dictionaryForKey:kAuralMonitor1 ] ;
	if ( d != nil ) [ [ [_auralMonitorPlist objectAtIndex:1] dictionary ] addEntriesFromDictionary:d ] ;

	d = [ pref dictionaryForKey:kAuralTransmitter ] ;
	if ( d != nil ) [ [ [_auralMonitorPlist objectAtIndex:2] dictionary ] addEntriesFromDictionary:d ] ;

	d = [ pref dictionaryForKey:kAuralWideband ] ;
	if ( d != nil ) [ [ [_auralMonitorPlist objectAtIndex:3] dictionary ] addEntriesFromDictionary:d ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating PSK configurations" ] ;
	[ (PSKConfig*)config updateFromPlist:pref ] ;
	
	[ self.manager showSplash:@"Loading PSK macros" ] ;
	for ( i = 0; i < 3; i++ ) {
		if ( macroSheet[i] ) {
...
- (void)retrieveForAuralMonitorPlist:(Preferences*)pref
{
	...
	[ pref setDictionary:[ [ _auralMonitorPlist objectAtIndex:0] dictionary ] forKey:kAuralMonitor0 ] ;
	[ pref setDictionary:[ [ _auralMonitorPlist objectAtIndex:1] dictionary ] forKey:kAuralMonitor1 ] ;
	[ pref setDictionary:[ [ _auralMonitorPlist objectAtIndex:2] dictionary ] forKey:kAuralTransmitter ] ;
	[ pref setDictionary:[ [ _auralMonitorPlist objectAtIndex:3] dictionary ] forKey:kAuralWideband ] ;
}
...
- (void)setTextColor:(NSColor*)inTextColor sentColor:(NSColor*)sentTColor backgroundColor:(NSColor*)bgColor plotColor:(NSColor*)pColor
{
	...
	[ receive1View setTextColor:inTextColor attribute:receive1TextAttribute ] ;
	[ receive2View setTextColor:inTextColor attribute:receive2TextAttribute ] ;
...
- (void)changeTransmitStateTo:(Boolean)state
{
	...
	if ( transmitState == YES ) {
		...
		if ( self.timeout ) [ self.timeout invalidate ] ;
		charactersSinceTimerStarted = 0 ;
		if ( [ self.manager useWatchdog ] ) [self setTimeout:[ NSTimer scheduledTimerWithTimeInterval:150 target:self selector:@selector(timedOut:) userInfo:self repeats:YES ] ];
		_transmitBufferCheck = [ NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(checkTransmitBuffer:) userInfo:self repeats:YES ] ;
		...
		if ( self.timeout ) [ self.timeout invalidate ] ;
		[ self setTimeout:nil];
		if ( _transmitBufferCheck ) [ _transmitBufferCheck invalidate ] ;
		_transmitBufferCheck = nil ;
		[ self ptt:NO ] ;
		[ self.transmitCountLock lock ] ;
		transmitCount = 0 ;
...
- (void)flushOutput
{
	...
	indexOfUntransmittedText = (int)[ [ transmitView textStorage ] length ] ;
...
- (BOOL)textView:(NSTextView *)textView shouldChangeTextInRange:(NSRange)original replacementString:(NSString*)replace
{
	...
	if ( textView == transmitView ) {
		...
		[ _transmitViewLock lock ] ;						//  v0.64
		storage = [ transmitView textStorage ] ;
		total = (int)[ storage length ] ;
		start = (int)original.location ;
...
{
	unmarkedTextLength = (int)insertionRange.location + (int)[ string length ] ;
}
