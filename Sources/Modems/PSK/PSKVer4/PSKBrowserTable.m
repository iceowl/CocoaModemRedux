...
@synthesize freqColumn  = _freqColumn;
@synthesize textColumn = _textColumn ;
@synthesize alarmText  = _alarmText;
@synthesize busy  = _busy;
@synthesize dot = _dot;
@synthesize null = _null;
@synthesize returnString = _returnString;
@synthesize attributedString  = _attributedString;
//  vfo offsets
@synthesize refreshCheck  = _refreshCheck;
//  alarms
@synthesize alarmString  = _alarmString;
@synthesize alarmLock = _alarmLock;
...
- (id)initWithTable:(NSTableView*)which client:(PSKBrowserHub*)client
{
	...
	self = [ super init ] ;
	if ( self ) {
		_alarmLock = [ [ NSLock alloc ] init ] ;
		_alarmString = nil ;
		alarmMask = NSCaseInsensitiveSearch ;
		[ [NSBundle mainBundle] loadNibNamed:@"PSKAlarms" owner:self topLevelObjects:nil ] ;
		
		_alarmText = [NSDictionary dictionaryWithObjectsAndKeys:[ NSColor redColor ], NSForegroundColorAttributeName, /*a->font, NSFontAttributeName,*/ nil ];
		
		_table = which ;
		_busy = [ [ NSLock alloc ] init ] ;
		hub = client ;
		
		columns = [ _table tableColumns ] ;
		_freqColumn = [ columns objectAtIndex:1 ] ;
		_textColumn = [ columns objectAtIndex:0 ] ;
		...
		_dot = @"." ;
		_null = @"" ;
		_returnString = [ [ NSMutableString alloc ] initWithCapacity:1024 ] ;
		[ _returnString setString:_null ] ;
		
		_attributedString = [ [ NSMutableAttributedString alloc ] initWithString:@"test 0123456789" ] ;
		
		 //  assume fixed width font
		NSFont *font = [ [ _textColumn dataCell ] font ] ;
		...
		refreshBusy = NO ;
		_refreshCheck = [ NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(refreshCheck:) userInfo:self repeats:YES ] ;
	}
	return self ;
}
...
- (void)alarmChanged
{
	...
	if ( [ string length ] > 0 ) _alarmString = [ [ NSString alloc ] initWithString:string ] ; else _alarmString = nil ;
...
- (void)refreshCheck:(NSTimer*)timer
{
	...
	for ( i = ( refreshCycle++ )&0x1; i < 21; i += 2 ) {
		...
		if ( r->refreshedCount < refreshNeeded ) {
			...
			[ _table setNeedsDisplayInRect:[ _table rectOfRow:tableRow ] ] ;
...
- (id)tableView:(NSTableView*)tableView objectValueForTableColumn:(NSTableColumn*)tableColumn row:(int)requestedRow
{
	...
	if ( tableColumn == _freqColumn ) {
		if ( sp == nil ) return _dot ;
		
		freq = sp->frequency;
		if ( freq < 0 ) return _dot ;
		
		freq -= vfoOffset ;
		if ( !isUSB ) freq = -freq ;
		
		str = [ [ NSString alloc ] initWithFormat:@"%d", (int)( freq + 0.5 ) ] ;
		[ _returnString setString:str ] ;
		//[ str release ] ;
	...
	if ( _alarmString == nil ) return str ;
	
	//  check for alarms
	[ _alarmLock lock ] ;
	
	range = [ str rangeOfString:_alarmString options:alarmMask ] ;
	...
	int length = (int)[ _attributedString length ] ;
	[ _attributedString removeAttribute:NSForegroundColorAttributeName range:NSMakeRange(0, 1) ] ;
	[ _attributedString replaceCharactersInRange:NSMakeRange( 0, length ) withString:str ] ;
	...
	for ( try = 0; try < 50; try++ ) {
		...
		[ _attributedString addAttribute:NSForegroundColorAttributeName value:[ NSColor redColor ] range:range ] ;
		...
		range = [ substr rangeOfString:_alarmString options:alarmMask ] ;
...
- (float)selectSlot:(int)slotIndex
{
	...
	[ _table selectRowIndexes:[ NSIndexSet indexSetWithIndex:mappedRowIndex ] byExtendingSelection:NO ] ;
