...
- (void)newSpectrum:(DSPSplitComplex*)spec size:(int)n
{
    ...
    if(_timer1 == nil) [self myCreateTimer];
    okToDraw = TRUE;
...

{
    ...
    __block FrequencyIndicator* bSelf = self; // capture so as to not have compiler complain about ARC retain cycles...dunno if this is going to be a problem.
    
    if(!_timerQueue)  _timerQueue = dispatch_queue_create("com.owlhousetoys.waterfallTimer", DISPATCH_QUEUE_SERIAL);
    if((!_timer1) && _timerQueue) {
        _timer1 = [self  CreateDispatchTimer :50ull * NSEC_PER_MSEC
                                             :10ull * NSEC_PER_MSEC
                                             :_timerQueue
                                             :gBlock = [^(void){
            if(*w_okToDraw){
                [bSelf displayInMainThread];
                *w_okToDraw = FALSE;
            }
            // usleep(10);
        } copy]];
...
-(void)killFreqIndTimer {
    
    if(_timer1) dispatch_source_cancel(_timer1);
}

-(void)dealloc {
    [self killFreqIndTimer];
    free(bitmaps[0]);
}
