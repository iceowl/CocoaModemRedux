...
- (void)importWidebandData:(CMPipe*)pipe 
{
	...
	[ self.auralMonitor addLeft:outbuf right:nil samples:512 client:self ] ;
}
...
- (void)importTransmitData:(float*)array 
{
	...
	if ( p->enabled && p->active ) {
		...
		[ self.auralMonitor addLeft:nil right:outbuf samples:512 client:self ] ;
	}
}
...
- (void)updateSamplingState
{
	...
	if ( wasSampling != isSampling ) {
		if ( isSampling == YES ) [ self.auralMonitor addClient:self ] ; else [ self.auralMonitor removeClient:self ] ;
