...
{
	if ( [ [NSBundle mainBundle] loadNibNamed:@"PSKReceiver" owner:self topLevelObjects:nil] ) {
...
- (void)receiveThread
{
    ...
    if(_PSKreceiveQueue == nil) _PSKreceiveQueue = dispatch_queue_create("com.owlhousetoys.PSKReceiveQueue", DISPATCH_QUEUE_CONCURRENT);
    
    __block CMDataStream* str = [self stream];
    
    dispatch_async(_PSKreceiveQueue, [ ^(void) {
