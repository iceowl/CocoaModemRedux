...
- (void)awakeFromNib
{
	[self setIdent: NSLocalizedString( @"Synch-AM", nil )] ;
...
{
	if ( [ (NSButton*)equalizerCheckbox state ] == NSOnState ) {
...
- (void)volumeChanged
{
	...
	if ( [(NSButton*) muteCheckbox state ] == NSOnState ) {
		v = 0.0 ;
	}
	else {
...
{
	[ self.manager showSplash:@"Updating Synchronous AM sound source" ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating Synchronous AM configurations" ] ;
	...
	[ (NSButton*)equalizerCheckbox setState:( [ pref intValueForKey:kSynchAMEqEnable ] == 0 ) ? NSOffState : NSOnState ] ;
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ pref setInt:( [ (NSButton*)equalizerCheckbox state ] == NSOffState ) ? 0 : 1 forKey:kSynchAMEqEnable ] ;
