...
- (void)setFEC
{
	...
	fecMode = (int)[ [ dominoFECMenu selectedItem ] tag ] ;
	useFEC = ( fecMode >= 4 ) ;
...
- (void)awakeFromNib
{
	[self setIdent :NSLocalizedString( @"MFSK", nil )] ;
...
- (void)setTextColor:(NSColor*)inTextColor sentColor:(NSColor*)sentTColor backgroundColor:(NSColor*)bgColor plotColor:(NSColor*)pColor
{
	...
	[self setSentTextColor :  sentTColor];
	...
	[ dominoReceiveView setBackgroundColor:self.backgroundColor ] ;
	[ dominoReceiveBox setBackgroundColor:self.backgroundColor ] ;
	[ dominoSendField setBackgroundColor:self.backgroundColor ] ;
	
	[ receiveView setTextColor:inTextColor attribute:receiveTextAttribute ] ;
	[ transmitView setViewTextColor:inTextColor attribute:transmitTextAttribute ] ;
	[ dominoReceiveView setTextColor:inTextColor attribute:receiveTextAttribute ] ;
	[ dominoReceiveBox setTextColor:inTextColor ] ;
	[ dominoSendField setTextColor:inTextColor ] ;
}
...
- (void)displayCharacter:(int)c
{
	...
	[ [ self.manager appObject ] addToVoice:c channel:1 ] ;		//  0.96d
...
- (void)changeTransmitStateTo:(Boolean)state
{
	...
	if ( transmitState == YES ) {
		...
		if (_timeout) [ _timeout invalidate ] ;
		charactersSinceTimerStarted = 0 ;
		//  prepare modulator
		[ modulator resetModulator ] ;
		if ( [ self.manager useWatchdog ] ) _timeout = [ NSTimer scheduledTimerWithTimeInterval:150 target:self selector:@selector(timedOut:) userInfo:self repeats:YES ] ;
...
- (void)transmittedCharacter:(int)c channel:(int)channel
{
	...
	[ [ self.manager appObject ] addToVoice:c channel:0 ] ;		//  0.96d
	//  applescript
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating MFSK configurations" ] ;
	...
	[ self.manager showSplash:@"Loading MFSK macros" ] ;
	for ( i = 0; i < 3; i++ ) {
		if ( macroSheet[i] ) {
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	i = (int)[ [ mfskModeMenu selectedItem ] tag ] ;
