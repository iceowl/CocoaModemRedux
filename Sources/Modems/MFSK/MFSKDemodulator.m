...
- (void)decodeThread
{
	...
	__block float *packed;
    
    packed = malloc(sizeof(float) * 64);
    if(_MFSKqueue == nil) _MFSKqueue = dispatch_queue_create("com.owlhousetoys.MFSKqueue", DISPATCH_QUEUE_CONCURRENT);
    dispatch_async(_MFSKqueue, [ ^(void) {
