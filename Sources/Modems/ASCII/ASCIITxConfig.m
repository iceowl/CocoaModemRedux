...
- (void)awakeFromModem:(RTTYConfigSet*)set rttyRxControl:(RTTYRxControl*)control
{
	...
	if ( set.outputDevice ) {
		...
		self.afsk = [ [ ASCIIModulator alloc ] init ] ;
...
- (void)setupDefaultPreferences:(Preferences*)pref rttyRxControl:(RTTYRxControl*)control
{
	...
	if ( self.configSet.stopBits ) [ pref setFloat:2.0 forKey:self.configSet.stopBits ] ;
	[ self set:self.configSet.sentColor fromRed:0.0 green:0.8 blue:1.0 into:pref ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref rttyRxControl:(RTTYRxControl*)control 
{
	...
	[ self updateColorsFromPreferences:(Preferences*)pref configSet:self.configSet ] ;
	...
	//  stop bits
	if ( self.configSet.stopBits ) stopValue = [ pref floatValueForKey:self.configSet.stopBits ] ;
	...
	if ( self.configSet.stopBits ) {
		if ( stopBits ) [ stopBits selectCellAtRow:index column:0 ] ;
		if ( self.afsk ) [ self.afsk setStopBits:stopValue ] ;
	}
	return true ;
}
...
- (void)retrieveForPlist:(Preferences*)pref rttyRxControl:(RTTYRxControl*)control
{
	...
	[ self set:self.configSet.sentColor fromColor:[ transmitTextColor color ] into:pref ] ;
	//  rtty output prefs
	...
	if ( stopBits ) {
		...
		[ pref setFloat:stopDuration[index] forKey:self.configSet.stopBits ] ;
	}
}
...
- (Boolean)startTransmit
{
	...
	if ( self.fsk ) return [ self startFSKTransmit ] ;		// v0.50
	...
	if ( !isTransmit && !configOpen ) {
		...
		[ self.afsk appendString:"|" clearExistingCharacters:NO ] ;  //  send a long mark
...
- (Boolean)stopFSKTransmit
{
	if ( isTransmit ) {
		isTransmit = NO ;
		[ self.fsk stopSampling ] ;
		if ( transmitButton ) {
			[ transmitButton setTitle:NSLocalizedString( @"Transmit", nil ) ] ;
			[ transmitButton setState:NSOffState ] ;
		}
		[ self.fsk clearOutput ] ;
...
{
	if ( self.fsk ) return [ self stopFSKTransmit ] ;		//  v0.50
...
- (void)transmitCharacter:(int)ascii
{
	...
	if ( self.fsk ) [ self.fsk appendASCII:ascii ] ; else [ self.afsk appendASCII:ascii ] ;		// v0.50
}

- (void)flushTransmitBuffer
{
	if ( self.fsk ) [ self.fsk clearOutput ] ; else [ self.afsk clearOutput ] ;					// v0.50
}
...
- (Boolean)turnOnTransmission:(Boolean)inState button:(NSButton*)button fsk:(FSK*)inFSK
{
	...
	if ( self.fsk != nil) {
		...
		fd = [ self.fsk useSelectedPort ] ;
		if ( fd <= 0 ) self.fsk = nil ;
	}
	ook = 0 ;
...
- (Boolean)turnOnTransmission:(Boolean)inState button:(NSButton*)button fsk:(FSK*)inFSK ook:(int)inOOK
{
	...
	self.fsk = inFSK ;
	if ( self.fsk != nil) {
		[ self.fsk setUSOS:usosState ] ;													//  v0.84
		//  select fsk port and check if port is good
		fd = [ self.fsk useSelectedPort ] ;
		if ( fd <= 0 ) self.fsk = nil ;
	}
...
- (void)selectTestTone:(int)index
{
	...
	case 4:
		toneIndex = index ;
		[ modemDest stopSampling ] ;
		[ self.afsk appendString:"========" clearExistingCharacters:YES ] ;
		[ modemDest startSampling ] ;
		break ;
	case 5:
		toneIndex = index ;
		[ modemDest stopSampling ] ;
		[ self.afsk appendString:"RYRYRYRY" clearExistingCharacters:YES ] ;
		[ modemDest startSampling ] ;
		break ;
	case 6:
		toneIndex = index ;
		[ modemDest stopSampling ] ;
		[ self.afsk appendString:"\nthe quick brown fox jumps over the lazy dog. 589 73 qrz" clearExistingCharacters:YES ] ;
		[ modemDest startSampling ] ;
		break ;
	default:
		toneIndex = index ;
...
- (int)needData:(float*)outbuf samples:(int)samples
{
	...
	case 0:
		...
		[ self.afsk getBufferWithDiddleFill:bpfBuf length:samples ] ;
		break ;
	case 1:
	default:
		[ self.afsk getBufferOfMarkTone:bpfBuf length:samples ] ;
		break ;
	case 2:
		[ self.afsk getBufferOfSpaceTone:bpfBuf length:samples ] ;
		break ;
	case 3:
		[ self.afsk getBufferOfTwoTone:bpfBuf length:samples ] ;
		break ;	
	case 4:
	case 5:
	case 6:
		[ self.afsk getBufferWithRepeatFill:bpfBuf length:samples ] ;
		break ;
	}
	
	//  apply bandpass filter and save into output
	CMPerformFIR( self.transmitBPF, bpfBuf, samples, outbuf ) ;
...
- (void)setOutputScale:(float)value
{
	...
	[ self.afsk setOutputScale:outputScale ] ;
}
