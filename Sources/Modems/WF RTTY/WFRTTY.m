...
- (id)initIntoTabView:(NSTabView*)tabview manager:(ModemManager*)mgr nib:(NSString*)nib
{
	...
	if ( self ) {
		...
		[ txConfig awakeFromModem:_setA rttyRxControl:nil ] ;
		...
		[ configB awakeFromModem:_setB rttyRxControl:b.control txConfig:txConfig ] ;	// note:shared txConfig
...
- (void)awakeFromNib
{
	[ self setIdent: ( isLite ) ? NSLocalizedString( @"RTTY", nil ) : NSLocalizedString( @"Wideband RTTY", nil )] ;
...
- (void)setupSpectrum
{
	...
	for ( i = 0; i < 2; i++ ) if ( waterfall[i] ) [ control[i] setTheWaterfall:waterfall[i] ] ;
}
...
{
	[ self.manager showSplash:@"Updating Wideband RTTY sound source" ] ;
...
{
	return ( [(NSButton*) [ transmitLock cellAtRow:0 column:index ] state ] == NSOnState ) ;
}
...
{
	[ (NSButton*)[ transmitLock cellAtRow:0 column:index ] setState:( locked ) ? NSOnState : NSOffState ] ;
}
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating WFRTTY configurations" ] ;
