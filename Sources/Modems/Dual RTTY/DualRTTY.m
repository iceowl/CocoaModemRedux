...
- (id)initIntoTabView:(NSTabView*)tabview manager:(ModemManager*)mgr
{
	...
	if ( self ) {
		...
		[ txConfig awakeFromModem:_setA rttyRxControl:a.control ] ;
		ptt = [ txConfig pttObject ] ;
		transmitChannel = 0 ;
        
        if(a == nil) a = [[RTTYTransceiver alloc] init];
        if(b == nil) b = [[RTTYTransceiver alloc] init];
		...
		[ configA awakeFromModem:_setA rttyRxControl:a.control txConfig:txConfig ] ;
		...
		[ (DualRTTYConfig*)configB awakeFromModem:_setB rttyRxControl:b.control txConfig:txConfig ] ;  // note:shared txConfig
...
- (void)awakeFromNib
{
	[self setIdent : NSLocalizedString( @"Dual RTTY", nil ) ] ;
...
- (void)setupSpectrum
{
	...
	case 0:
		[ b.control setTheSpectrumView:nil ] ;
		[ b.control setTheWaterfall:nil ] ;
		[ a.control setTheSpectrumView:spectrum ] ;
		[ a.control setTheWaterfall:waterfall ] ;
		break ;
	case 1:
		[ a.control setTheSpectrumView:nil ] ;
		[ a.control setTheWaterfall:nil ] ;
		[ b.control setTheSpectrumView:spectrum ] ;
		[ b.control setTheWaterfall:waterfall ] ;
		break ;
	default:
		// off
		[ a.control setTheSpectrumView:nil ] ;
	...
	t = (int) [ timeConstant indexOfSelectedItem ] ;
	tc = timeConstants[t] ;
	if ( tc < timeConstants[1] ) {
		...
		dr = (int)[ dynamicRange indexOfSelectedItem ] ;
...
{
	[ self.manager showSplash:@"Updating Dual RTTY sound source" ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating DualRTTY configurations" ] ;
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ pref setInt:(int)[ timeConstant indexOfSelectedItem ] forKey:kDualRTTYSpectrumDecay ] ;
	[ pref setInt:(int)[ dynamicRange indexOfSelectedItem ] forKey:kDualRTTYSpectrumRange ] ;
	[ pref setInt:(int)[ channel indexOfSelectedItem ] forKey:kDualRTTYSpectrumChannel ] ;
...
- (void)transmitSelectChanged
{
	...
	index = (int)[ transmitSelect selectedColumn ] ;
	//  move interfaces
	if ( index == 0 ) {
...
- (void)contestTransmitSelectChanged
{
	...
	index = (int)[ contestTransmitSelect selectedColumn ] ;
