...
- (id)initWithCapacity:(int)size ident:(NSString*)inIdent
{
	...
	if ( self ) {
        ...
        if(size > MAXELEMENTS) {
            NSLog(@" Asking for Linked Array %d can only accomodate %d", size, MAXELEMENTS);
            size = MAXELEMENTS;
        }
        ...
        for (int i =0; i< capacity;i++) idle[i] = [[DataElement alloc]init];
		elements = 0 ;
...
- (DataElement*)assign:(id)object next:(DataElement*)next
{
	...
	if ( elements >= capacity ) {
		...
		p = nil ;
		//p->next = nil ; this will create disaster
		return p ;
	}
...
- (void)removeObjectAtIndex:(int)index
{
	...
	#ifdef DEBUGARRAY
	 printf( "%s:Linked Array removeObjectAtIndex %d out of %d\n", [ ident UTF8String ], index, elements ) ;
	 #endif
