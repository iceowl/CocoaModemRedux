...
@synthesize sentTextColor       = _sentTextColor;
@synthesize plotColor           = _plotColor;
@synthesize getCallLock         = _getCallLock;
@synthesize waterfallDate       = _waterfallDate;
@synthesize waterfallLock       = _waterfallLock;
@synthesize timeout             = _timeout;
@synthesize modemTabItem        = _modemTabItem;
@synthesize controllingTabView  = _controllingTabView;
@synthesize ident               = _ident;
@synthesize manager             = _manager;
@synthesize transmitCountLock   = _transmitCountLock;
...
- (id)initWithManager:(ModemManager*)mgr 
{
	...
	if ( self ) {
		...
		_getCallLock = [ [ NSLock alloc ] init ] ;
		_waterfallDate = [ [ NSDate alloc ] init ] ;		//  v0.82
	}
	return self ;
}
...
{
	self = [ self initWithManager:mgr ] ;

	if ( self ) {
		ptt = nil ;
		if ( [ [NSBundle mainBundle] loadNibNamed:nib owner:self topLevelObjects:nil] ) {
			// loadNib should have set up modem view
			if ( view ) {
				//  create a new TabViewItem for modem
				_modemTabItem = [ [ NSTabViewItem alloc ] init ] ;
				[ _modemTabItem setView:view ] ;
				[ _modemTabItem setLabel:_ident ] ;		// ident set from awakeFromNib
				//  and insert as tabView item
				_controllingTabView = tabview ;
				[ _controllingTabView insertTabViewItem:_modemTabItem atIndex:0 ] ;
...
- (void)initColors
{
	...
	_sentTextColor = [NSColor blackColor ] ;
	_textColor = [ NSColor blackColor ] ;
	_backgroundColor = [NSColor whiteColor ] ;
	_plotColor = [ NSColor greenColor ]  ;
}
...
- (void)setTextColor:(NSColor*)inTextColor sentColor:(NSColor*)sentTColor backgroundColor:(NSColor*)bgColor plotColor:(NSColor*)pColor
{
	...
	[ receiveView setBackgroundColor:_backgroundColor ] ;
	[ transmitView setBackgroundColor:_backgroundColor ] ;
	
	[ receiveView setTextColor:_textColor attribute:receiveTextAttribute ] ;
	[ transmitView setViewTextColor:_textColor attribute:transmitTextAttribute ] ;
}
...
- (void)setSentColor:(Boolean)state view:(ExchangeView*)eview textAttribute:(TextAttribute*)attr
{
	if ( attr != nil ) {
		if ( state == YES ) {
			if ( sentColor != YES ) {
				[ eview setTextColor:_sentTextColor attribute:attr ] ;
				sentColor = YES ;
			}
		}
		else {
			[ eview setTextColor:_textColor attribute:attr ] ;
			sentColor = NO ;
		}
	}
}
...
{
	printf( "%s did not implement -dataClient\n", [ _ident cStringUsingEncoding:NSASCIIStringEncoding] ) ;
	return nil ;
}
...
{
	[ _transmitCountLock lock ] ;
	transmitCount++ ;
	[ _transmitCountLock unlock ] ;
}
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ _manager clearToolTipsInView:view ] ;
}

- (NSTabViewItem*)tabItem
{
	return _modemTabItem ;
}

- (Boolean)isActiveTab
{
	return ( [ _manager activeTabView ] == _modemTabItem ) ;
}
...
- (void)upperCase:(NSString*)string into:(char*)cstr
{
	...
	if ( n > 30 ) {
		...
		strncpy( cstr, [ string cStringUsingEncoding:NSASCIIStringEncoding  ], 30 ) ;
	} 
	else {
		strcpy( cstr, [ string cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
		for ( i = 0; i < n; i++ ) {
			v = cstr[i] & 0xff ;													// v0.25
...
- (void)captureSelection:(NSTextView*)textView
{
	...
	if ( range.length > 0 ) {
		storage = [ textView textStorage ] ;
		string = [ [ storage attributedSubstringFromRange:range ] string ] ;
		[ [ _manager appObject ] saveSelectedString:string view:textView ] ;
	}
}
...
- (void)setStream:(NSString*)text
{
	...
	index = 0 ;
	s = t = (char*)[ text cStringUsingEncoding:NSASCIIStringEncoding  ] ;
...
- (void)newFFTBuffer:(float*)buffer
{
	...
	waterfallTouched = -[ _waterfallDate timeIntervalSinceNow ] ;		//  v0.82
...
- (int)nextWaterfallScanline
{
	...
	elapsed = -[ _waterfallDate timeIntervalSinceNow ] - waterfallTouched ;
