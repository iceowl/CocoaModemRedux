...
@synthesize vuOffColor = _vuOffColor;
@synthesize vu         = _vu;
-(id) init   {
    
    self = [super init];
    if(self) {
        _vu = [[NSMutableArray alloc] initWithCapacity:NUMELEMENTS];
        for(int q = 0;q < NUMELEMENTS;q++)[_vu insertObject:[[VUElement alloc] init] atIndex:q];
    }
...
- (void)setup 
{
	...
	//  VU meter
	_vuOffColor = [ NSColor colorWithCalibratedRed:0.25 green:0.25 blue:0.25 alpha:1 ] ;
	for ( i = 0; i < 9; i++ ) {
        VUElement *vue = [_vu objectAtIndex:i];
		vue.segment = [ matrix cellAtRow:0 column:i ] ;
		vue.state = NO ;
		if ( i == 7 ) vue.onColor = [ NSColor colorWithCalibratedRed:0.9 green:0.9 blue:0 alpha:1 ] ;
		else if ( i == 8 ) vue.onColor = [ NSColor colorWithCalibratedRed:0.85 green:0 blue:0 alpha:1 ] ;
		else vue.onColor = [ NSColor colorWithCalibratedRed:0 green:0.7 blue:0 alpha:1 ] ;
		//[ vu[i].onColor retain ] ;
		[ vue.segment setBackgroundColor:_vuOffColor ] ;
	}
	...
	if ( [ overrunLock tryLock ] ) {
		...
		for ( i = 0; i < 9 ; i++ ) {
			...
			if ( n >= (i+1) ) {
				...
				if ( vi.state == YES ) {
					[ vi.segment setBackgroundColor:_vuOffColor ] ;
					vi.state = NO ;
					hasChange = YES ;
				}
			}
