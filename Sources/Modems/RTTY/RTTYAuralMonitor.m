...
- (void)updateAuralMonitorState
{
	if ( ( muted == NO ) && ( demodulatorIsActive == YES ) ) {
		if ( monitorIsActive == NO ) [ self.auralMonitor addClient:self ] ;
		monitorIsActive = YES ;
	}
	else {
		if ( monitorIsActive == YES ) [  self.auralMonitor removeClient:self ] ;
		monitorIsActive = NO ;
	}
}
...
{
	if ( isReceiver ) {
		[ self.auralMonitor addLeft:si right:nil samples:512 client:self ] ;
	}
	else {
		[ self.auralMonitor addLeft:nil right:si samples:512 client:self ] ;
	}
}
...
- (void)newBandpassFilteredData:(float*)array scale:(float)scale fromReceiver:(Boolean)fromReceiver
{
	...
	if ( self.auralMonitor == nil || monitorIsActive == NO ) return ;
...
- (void)newWidebandData:(CMPipe*)pipe
{
	...
	if ( rxBackgroundOn == NO || self.auralMonitor == nil || monitorIsActive == NO ) return ;
