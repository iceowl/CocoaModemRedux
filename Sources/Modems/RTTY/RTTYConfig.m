...
- (void)afskMenuChanged
{
	...
	[ (RTTY*)modemObj afskChanged:(int)[ afskMenu indexOfSelectedItem ] config:self ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref rttyRxControl:(RTTYRxControl*)control
{
	...
	if ( ( ![ modemSource updateFromPlist:pref ] ) && ( [(NSButton*) activeButton state ] == NSOnState ) ) {
	...
	[ (NSButton*)activeButton setState:state ] ;
	...
	if ( prefMatrix ) {
		...
		count = (int)[ prefMatrix numberOfRows ] ;
		if ( s ) {
			rttyString = [ s cStringUsingEncoding:NSASCIIStringEncoding  ] ;
			for ( i = 0; i < count; i++ ) {
				state = rttyString[i] ;
				if ( state == 0 ) break ;
...
- (void)retrieveForPlist:(Preferences*)pref rttyRxControl:(RTTYRxControl*)control
{
	...
	if ( fileSpeedCheckbox ) [ pref setInt:( ( [ (NSButton*)fileSpeedCheckbox state ] == NSOnState ) ? 1 : 0 ) forKey:kFastPlayback ] ;
	...
	if ( [ [ NSString stringWithCString:str encoding:NSASCIIStringEncoding ] length ] == 0 ) {
		[ pref setString:@"" forKey:configSet.prefs ] ;
	}
	else {
		[ pref setString:[ NSString stringWithCString:str encoding:NSASCIIStringEncoding] forKey:configSet.prefs ] ;
	}
    
	//  active button states and local flag
	[ pref setInt:( [ (NSButton*)activeButton state ] == NSOnState )? 1 : 0 forKey:configSet.active ] ;
