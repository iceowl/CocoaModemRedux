...
- (void)importData:(CMPipe*)pipe
{
	...
	if ( self.auralMonitor ) {
		stream = [ pipe stream ] ;
		array = stream->array ;
		[ self.auralMonitor newBandpassFilteredData:array scale:1.0 fromReceiver:YES ] ;
	}
}
...
- (void)setTonePair:(const CMTonePair*)tonepair
{
	...
	if ( self.auralMonitor ) [ self.auralMonitor setTonePair:tonepair ] ;
}
