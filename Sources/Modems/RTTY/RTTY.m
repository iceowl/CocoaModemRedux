...
- (id)initIntoTabView:(NSTabView*)tabview manager:(ModemManager*)mgr
{
	...
	if ( self ) {
		...
		[ txConfig awakeFromModem:_set rttyRxControl:a.control ] ;
		...
		[ a.control setName: @"Main Receiver" ] ;
		[ a.control useAsTransmitTonePair:YES ] ;

		[ config awakeFromModem:_set rttyRxControl:ctrl txConfig:txConfig ] ;
...
- (void)awakeFromNib
{
	[self setIdent:NSLocalizedString( @"RTTY", nil ) ];
...
{
	[ self.manager showSplash:@"Updating RTTY sound source" ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating RTTY configurations" ] ;
