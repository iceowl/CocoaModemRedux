...
- (id)initIntoTabView:(NSTabView*)tabview manager:(ModemManager*)mgr
{
	...
	if ( self ) {
        ...
        a = [[RTTYTransceiver alloc] init];
        b = [[RTTYTransceiver alloc] init];
...
- (void)setRTTYPrefs:(NSMatrix*)rttyPrefs channel:(int)channel
{
	...
	count = (int)[ rttyPrefs numberOfRows ] ;
	if ( count > 1 ) {
		for ( i = 0; i < count; i++ ) {
...
{
	[ self.manager updateRTTYMacroButtons ] ;		// this will cause both RTTY and DualRTTY buutons to be updated
}
...
- (void)insertTransmittedCharacter:(int)c
{
	...
	[ [ self.manager appObject ] addToVoice:c channel:0 ] ;		//  v0.96d	voice synthesizer
...
{
	[ self selectDemodulator:(int)[ sender selectedColumn ] ] ;
}
...
- (void)enableModem:(Boolean)active
{
	...
	if ( active == YES ) {
		...
		if ( contestBar ) {
			...
			[ contestBar setModem:(ContestInterface*)[ self.manager currentModem ] ] ;			//  v0.33
...
- (BOOL)textView:(NSTextView *)textView shouldChangeTextInRange:(NSRange)original replacementString:(NSString *)replace
{
	...
	if ( textView == transmitView ) {
		...
		if ( storage != nil ) {				//  sanity check v0.64c
			...
			length = (int)[ [ storage attributedSubstringFromRange:original ] length ] ;				//  v0.65 - get length instaed of byte count in attributed string
