...
- (void)awakeFromNib
{
	...
	[self setIdent: NSLocalizedString( @"HF-FAX", nil ) ];
...
{
	[ self.manager showSplash:@"Updating HF-FAX sound source" ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating HF-FAX configurations" ] ;
...
{
	if ( rx ) [ rx changeBandwidthTo:(int)[ bandwidthMenu indexOfSelectedItem ] ] ;
}
