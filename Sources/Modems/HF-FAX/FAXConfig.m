...
{
	[ [ (FAX*)modemObj faxView ] setDeviation: ( [ (NSButton*)deviationCheckbox state ] == NSOnState )? 1 : 0 ] ;
}
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ (NSButton*)activeButton setState:( [ pref intValueForKey:kFAXActive ] == 1 ) ? NSOnState : NSOffState ] ;
	...
	[(NSButton*) deviationCheckbox setState:( state ) ? NSOnState : NSOffState ] ;
	[ faxView setDeviation:state ] ;

	//  always fast file speed
	[(NSButton*) fileSpeedCheckbox setState:NSOnState ] ;
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ pref setInt:( [ (NSButton*)activeButton state ] == NSOnState )? 1 : 0 forKey:kFAXActive ] ;
	
	[ pref setInt:( [ (NSButton*)deviationCheckbox state ] == NSOnState )? 1 : 0 forKey:kFAXDeviation ] ;
}
