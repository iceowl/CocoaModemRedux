...
@synthesize bitmapRep = _bitmapRep;
@synthesize imageLock = _imageLock;
@synthesize bitmapRepLock = _bitmapRepLock;
@synthesize updateLock = _updateLock;
@synthesize dumpLock = _dumpLock;
@synthesize alternateFolder = _alternateFolder;
...
- (id)initWidth:(int)w height:(int)h
{
	...
	if ( self ) {
		...
		scrollOffset = 0 ;
		_bitmapRep = [ self createNewImageRep:pixelBuffer topOffset:scrollOffset height:height ] ;
		_bitmapRepLock = [ [ NSLock alloc ] init ] ;
		...
		[ _image addRepresentation:_bitmapRep ] ;
		_imageLock = [ [ NSLock alloc ] init ] ;
		_dumpLock = [ [ NSLock alloc ] init ] ;
		
		_alternateFolder = nil ;
	}
	return self ;
}
...
- (void)swapImageRep
{
	[ _imageLock lock ] ;
	// remove the old rep
	[ _image removeRepresentation:_bitmapRep ] ;
	//	and replace with a new one
	[ _image addRepresentation:[ self updateGrayImageRep ] ] ;
	[ _imageLock unlock ] ;
}
...
- (void)dumpFrame:(NSString*)folder
{
	...
	NSString *nfolder = [ folder stringByExpandingTildeInPath ] ;
	if ( [ [ NSFileManager defaultManager ] fileExistsAtPath:nfolder isDirectory:&isDirectory ] == NO || isDirectory == NO ) {
		button = (int)[ [ NSAlert alertWithMessageText:@"FAX folder not found.  Save to your home directory instead?" defaultButton:NSLocalizedString( @"OK", nil ) alternateButton:NSLocalizedString( @"Cancel", nil ) otherButton:nil informativeTextWithFormat:@"\nFolder %@ is not found.\n", nfolder  ] runModal ] ;
		if ( button != 1 ) {
			free( imageBuffer ) ;
			return ;
		}
		folder = _alternateFolder = [ @"~" stringByExpandingTildeInPath ] ;
	}
...
- (void)dumpCurrentFrameToFolder:(NSString*)folder
{
	...
	[ _dumpLock lock ] ;
	if ( _alternateFolder != nil ) folder = _alternateFolder ;
	if ( folder == nil || [ folder length ] <= 0 ) {
		button =(int) [ [ NSAlert alertWithMessageText:@"FAX folder has not been set up.  Save to your home directory instead?" defaultButton:NSLocalizedString( @"OK", nil ) alternateButton:NSLocalizedString( @"Cancel", nil ) otherButton:nil informativeTextWithFormat:@"" ] runModal ] ;
		if ( button != 1 ) {
			[ _dumpLock unlock ] ;
			return ;
		}
		_alternateFolder = folder =  [ @"~" stringByExpandingTildeInPath ]  ;
	}
