...
- (void)pullThread
{
	...
	__block int i;
	__block CMAnalyticPair pair ;
	__block float u, v, freq, iDot, qDot, *inputBuffer ;
    
    inputBuffer = malloc(sizeof(float) * 512);
    
    //	runLoopCycle = 0 ;
	
    if(_faxQueue == nil) _faxQueue = dispatch_queue_create("com.owlhousetoys.FAXQueue", DISPATCH_QUEUE_CONCURRENT);
    dispatch_async(_faxQueue, [ ^(void) {
