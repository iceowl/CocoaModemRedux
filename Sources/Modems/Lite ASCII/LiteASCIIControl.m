...
- (id)initIntoView:(NSView*)view client:(Modem*)modem index:(int)index
{
	self = [ super init ] ;
	if ( self ) {
		if ( [ [NSBundle mainBundle] loadNibNamed:@"LiteASCIIControl" owner:self topLevelObjects:nil] ) {
...
- (void)importData:(CMPipe*)pipe
{
	...
	[ (LiteASCII*)self.client drawSpectrum:pipe ] ;
}
...
- (void)setTonePair:(const CMTonePair*)tonepair mask:(int)mask
{
	...
	if ( mask & 1 ) [ (LiteASCII*)self.client changeMarkersInSpectrum:self ] ;
}
