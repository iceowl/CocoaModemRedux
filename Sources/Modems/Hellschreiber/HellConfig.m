...
- (void)awakeFromModem:(Hellschreiber*)modem
{
	...
	fnt = MakeHellFont( [ path cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
	if ( fnt ){
		font[fonts] = fnt ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ (NSButton*)activeButton setState:( [ pref intValueForKey:kHellActive ] == 1 ) ? NSOnState : NSOffState ] ;
	
	//  now reset active states if autoconnect is off
	if ( [ pref intValueForKey:kAutoConnect ] == 0 ) [ (NSButton*)activeButton setState:NSOffState ] ;
	
	[ self updateColorsFromPreferences:(Preferences*)pref ] ;
	
	if ( ( ![ modemSource updateFromPlist:pref ] || ![ modemDest updateFromPlist:pref ] ) && ( [(NSButton*) activeButton state ] == NSOnState ) ) {
		[(NSButton*) activeButton setState:NSOffState ] ;
	...
	[ (NSButton*)fileSpeedCheckbox setState:( state ) ? NSOnState : NSOffState ] ;
	[ self updateFileSpeed ] ;
	
	state = [ pref intValueForKey:kHellDiddle ] ;
	[(NSButton*) diddleButton setState:( state ) ? NSOnState : NSOffState ] ;
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ pref setInt:( ( [ (NSButton*)fileSpeedCheckbox state ] == NSOnState ) ? 1 : 0 ) forKey:kFastPlayback ] ;
	...
	[ pref setInt:( [ (NSButton*)activeButton state ] == NSOnState )? 1 : 0 forKey:kHellActive ] ;
	[ pref setInt:( [ (NSButton*)diddleButton state ] == NSOnState )? 1 : 0 forKey:kHellDiddle ] ;
	index = (int)[ sidebandMenu indexOfSelectedItem ] ;
...
- (void)updateDialOffset
{
	...
	[ modemObj setWaterfallOffset:offset sideband:(int)[ sidebandMenu indexOfSelectedItem ] ] ;
}
...
{
	[ oscilloscope setDisplayStyle:(int)[ waveformMatrix selectedRow ] plotColor:[ plotColor color ] ] ;
...
{
	[ self setSideband:(int)[ sidebandMenu indexOfSelectedItem ] ] ;
	[ self updateDialOffset ] ;
}
...
- (void)diddleChanged
{
	[ hellModulator setDiddle:[ (NSButton*)diddleButton state ] ] ;
}
