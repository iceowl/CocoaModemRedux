...
@synthesize receiveWait         = _receiveWait;
...
- (void)awakeFromNib
{
	...
	[self setIdent: NSLocalizedString( @"Hellschreiber", nil )] ;
    ...
    [ scroller setDoubleValue:1.0];
	[ scroller setKnobProportion: 0.125 ] ;
...
{
	[ self.manager showSplash:@"Updating Hellschreiber sound source" ] ;
...
- (Boolean)updateFromPlist:(Preferences*)pref
{
	...
	[ self.manager showSplash:@"Updating Hellschreiber configurations" ] ;
	[ (HellConfig*)config updateFromPlist:pref ] ;
	
	[ self.manager showSplash:@"Loading Hellschreiber macros" ] ;
	for ( i = 0; i < 3; i++ ) {
		if ( macroSheet[i] ) {
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ pref setString:[ NSString stringWithCString:font[unalignedFont]->name encoding:NSASCIIStringEncoding] forKey:kHellAlignedFont ] ;
	[ pref setString:[ NSString stringWithCString:font[unalignedFont]->name encoding:NSASCIIStringEncoding]  forKey:kHellUnalignedFont ] ;
...
- (void)changeTransmitStateTo:(Boolean)state
{
	...
	if ( transmitState == YES ) {
		...
		if ( self.timeout ) [ self.timeout invalidate ] ;
		[self setTimeout:nil];
		if ( _receiveWait ) [ _receiveWait invalidate ] ;
		_receiveWait = nil ;
...
- (void)transmitButtonChanged
{
	...
	state = ( [(NSButton*) transmitButton state ] == NSOnState ) ;
	
	if ( state ) {
		if ( ![ config soundInputActive ] ) {
			//  check if A/D is active
			[(NSButton*) transmitButton setState:NSOffState ] ;
...
- (BOOL)textView:(NSTextView *)textView shouldChangeTextInRange:(NSRange)original replacementString:(NSString *)replace
{
	...
	if ( textView == transmitView ) {
		if ( slashZero ) {
			s = ( char* )[ replace cStringUsingEncoding:NSASCIIStringEncoding  ] ;
			if ( s == nil ) {
			...
			if ( hasZero ) {
				...
				if ( length < 32 ) {
					...
					[ transmitView replaceCharactersInRange:original withString:[ NSString stringWithCString:replacement encoding:NSASCIIStringEncoding] ] ;
					return NO ;
				}
			}
		}
		...
		if ( length > 0 ) {
			if ( ( start+length ) == total ) {
				...
				if ( ( replacement == 168 ) ||		// opt U
					...
					//printf( "replace by %s\n", [ replace cStringUsingEncoding:NSASCIIStringEncoding  ] ) ;
					return YES ;
				}
...
- (void)setTextColor:(NSColor*)inTextColor :(Boolean*)sentTColor :(NSColor*)bgColor :(NSColor*)pColor
{
	//[ textColor release ] ;
	[self setTextColor:inTextColor];
//	[ sentTextColor release ] ;
	[self setSentColor: *sentTColor];
	
	//[ backgroundColor release ] ;
	[self setBackgroundColor: bgColor ] ;
	
	//[ plotColor release ] ;
	[self setPlotColor:pColor] ;
	
	[ receiveView setBackgroundColor:self.backgroundColor ] ;
	[ transmitView setBackgroundColor:self.backgroundColor ] ;
	
	[ receiveView setTextColors:self.textColor transmit:self.sentTextColor ] ;
	[ transmitView setViewTextColor:self.textColor attribute:transmitTextAttribute ] ;
}
...
- (void)setAlignedFont:(NSString*)name
{
	...
	for ( i = 0; i < fonts; i++ ) {
		if ( [ name isEqualToString:[ NSString stringWithCString: font[i]->name encoding:NSASCIIStringEncoding ] ] ) {
...
- (void)setUnalignedFont:(NSString*)name
{
	...
	unalignedFont = (int)[ fontMenu indexOfSelectedItem ] ;
...
- (void)addFont:(HellschreiberFontHeader*)inFont index:(int)index
{
	...
	[ fontMenu insertItemWithTitle:[ NSString stringWithCString:inFont->name encoding:NSASCIIStringEncoding] atIndex:index ] ;
}
...
- (void)fontChanged
{
	...
	index = (int)[ fontMenu indexOfSelectedItem ] ;
...
- (void)modeChanged
{
	...
	mode = (int)[ [ modeMenu selectedItem ] tag ] ;
