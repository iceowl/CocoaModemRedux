...
- (id)initIntoTabView:(NSTabView*)tabview manager:(ModemManager*)mgr
{
	...
	if ( self ) {
		...
		[ configA awakeFromModem:_setA rttyRxControl:a.control txConfig:nil ] ;
		...
		[ configB awakeFromModem:_setB rttyRxControl:b.control txConfig:nil ] ;
...
- (void)awakeFromNib
{
	[self setIdent: NSLocalizedString( @"SITOR-B", nil ) ];
...
- (void)setupSpectrum
{
	[ a.control setTheWaterfall:waterfallA ] ;
	[ b.control setTheWaterfall:waterfallB ] ;
}

- (void)updateSourceFromConfigInfo
{
	[ self.manager showSplash:@"Updating SITOR-B sound source" ] ;
...
- (void)tonePairChanged:(RTTYRxControl*)ctrl
{
	...
	if ( ctrl == a.control ) {
		[ (Waterfall*)waterfallA setSideband:sideband ] ;
		[ waterfallA setTonePairMarker:&tonepair index:0 ] ; 
	}
	else {
		[ (Waterfall*)waterfallB setSideband:sideband ] ;
		[ waterfallB setTonePairMarker:&tonepair index:1 ] ;
	}
}
