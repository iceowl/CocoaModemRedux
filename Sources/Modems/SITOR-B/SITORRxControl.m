...
@synthesize waitColor  = _waitColor;
@synthesize offColor   = _offColor;
@synthesize errorColor = _errorColor;
@synthesize fecColor   = _fecColor;

//@property (retain) NSColor *onColor, *waitColor, *offColor, *errorColor, *fecColor ;
...
- (id)initIntoView:(NSView*)view client:(Modem*)modem index:(int)index
{
	self = [ super init ] ;
	if ( self ) {
		if ( [ [NSBundle mainBundle] loadNibNamed:@"SITORRxControl" owner:self topLevelObjects:nil ] ) {
...
//	monitor = nil ;
	...
	_onColor = [ NSColor greenColor ] ;
	_waitColor = [ NSColor yellowColor ] ;
	_fecColor = [ NSColor orangeColor ] ;
	_errorColor = [ NSColor redColor ] ;
	_offColor = [ NSColor grayColor ] ;
...
- (void)setupWithClient:(Modem*)modem index:(int)index
{
	...
	self.receiver = [ [ SITORReceiver alloc ] initReceiver:index modem:modem ] ;
	//  set up receiver connections
	self.monitor = [ [ RTTYMonitor alloc ] init ] ;
	[ self.monitor setTitle:@"SITOR Monitor" ] ;
	
	[ (SITORReceiver*)self.receiver setControl:self ] ;
	[ self.receiver setSquelch:squelchSlider ] ;
	[ self.receiver setReceiveView:exchangeView ] ;
	[ self.receiver setDemodulatorModeMatrix:demodulatorModeMatrix ] ;
	[ self.receiver setBandwidthMatrix:bandwidthMatrix ] ;
}
...
- (void)setupDefaultPreferences:(Preferences*)pref config:(ModemConfig*)cfg
{
	...
	sidebandMenu = [ (RTTYConfig*)self.config sidebandMenu ] ;
...
- (void)updateFromPlist:(Preferences*)pref config:(ModemConfig*)cfg 
{
	...
	if ( self.receiver ) [ self.receiver setSquelchValue:[ pref floatValueForKey:set.squelch ] ] ;
...
- (void)retrieveForPlist:(Preferences*)pref config:(ModemConfig*)cfg
{
	...
	if ( self.receiver ) [ pref setFloat:[ self.receiver squelchValue ] forKey:set.squelch ] ;
}
