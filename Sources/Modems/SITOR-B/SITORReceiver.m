...
- (id)initReceiver:(int)index modem:(Modem*)modem
{
	...
	if ( self ) {
		...
		demodulator = (RTTYDemodulator*)[ [ SITORDemodulator alloc ] initFromReceiver:self ] ;		//  v0.79 bugfix -- was calling init instead of initFromReceiver
...
{
	if ( self.squelch ) {
		[ self.squelch setFloatValue:value ] ;
