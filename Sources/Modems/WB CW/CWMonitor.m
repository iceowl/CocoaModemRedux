...
@synthesize localLeftBuf = _localLeftBuf;
@synthesize localRightBuf= _localRightBuf;
@synthesize sidetoneBuf  = _sidetoneBuf;
@synthesize panoBuf      = _panoBuf;
...
- (id)init
{
	...
	if ( self ) {
		...
		auralLockout = 0 ;
        _panoBuf       = malloc(512*sizeof(float));
        _sidetoneBuf   = malloc(1024*sizeof(float));
        _localRightBuf = malloc(512*sizeof(float));
        _localLeftBuf  = malloc(512*sizeof(float));
		memset( _localLeftBuf, 0, 512*sizeof( float ) ) ;
		memset( _localRightBuf, 0, 512*sizeof( float ) ) ;
        memset( _panoBuf, 0, 512*sizeof(float));
        memset( _sidetoneBuf, 0, 1024*(sizeof(float)));
...
{
	mainChannels = (int)[ [ mainChannel selectedItem ] tag ] ;
	subChannels = (int)[ [ subChannel selectedItem ] tag ] ;
	txChannels = (int)[ [ txChannel selectedItem ] tag ] ;
}
...
- (void)updateSoundState
{
    ...
    //NSLog(@"sound state update to %d", state);
...
- (void)addLeft:(float*)leftbuf right:(float*)rightbuf state:(int)pushState
{
	...
	if ( accumulatedRx == 0 && accumulatedTx == 0 ) {
		memcpy( _localLeftBuf, leftbuf, 512*sizeof( float ) ) ;
		memcpy( _localRightBuf, rightbuf, 512*sizeof( float ) ) ;
		if ( pushState == 1 ) accumulatedRx = 1 ; else accumulatedTx = 1 ;
		return ;
	}
	if ( pushState == 1 && accumulatedRx > 0 ) {
		//  flush
		[ auralMonitor addLeft:_localLeftBuf right:_localRightBuf samples:512 client:self ] ;
		memcpy( _localLeftBuf, leftbuf, 512*sizeof( float ) ) ;
		memcpy( _localRightBuf, rightbuf, 512*sizeof( float ) ) ;
		accumulatedRx = 1 ;
		accumulatedTx = 0 ;
	...
	if ( ( pushState == 1 && accumulatedTx > 0 ) || ( pushState == 2 && accumulatedRx > 0 ) ) {
		for ( i = 0; i < 512; i++ ) {
			_localLeftBuf[i] += leftbuf[i] ;
			_localRightBuf[i] += rightbuf[i] ;
		}
		//  flush
		[ auralMonitor addLeft:_localLeftBuf right:_localRightBuf samples:512 client:self ] ;
		accumulatedRx = accumulatedTx = 0 ;
...
- (void)push:(float*)inph quadrature:(float*)quad wide:(float*)wide samples:(int)n
{
    ...
    memset( leftOutbuf, 0, sizeof(float)*512 ) ;
	memset( rightOutbuf, 0, sizeof(float)*512 ) ;

		//  main channel
	if ( isEnabled[0] ) {
		if ( isWide[0] && isPano[0] ) {
			// pano (reverse for LSB)
			[ receiver[0] needSidetone:_panoBuf inphase:inph quadrature:quad wide:wide samples:n wide:YES ] ;
			CMPerformFIR( panoLow, _panoBuf, 512, _sidetoneBuf ) ;
			CMPerformFIR( panoHigh, _panoBuf, 512, _sidetoneBuf+512 ) ;
			
			//  blend for separation
			t = 1.0 - separation ;
			for ( i = 0; i < 512; i++ ) {
				save = _sidetoneBuf[i] ;
				_sidetoneBuf[i] = separation*_sidetoneBuf[i] + t*_sidetoneBuf[i+512] ;
				_sidetoneBuf[i+512] = separation*_sidetoneBuf[i+512] + t*save ;
			}			
			//  left channel
			offset = ( panoReversed ) ? 512 : 0 ;
			gain = monitorGain[0] * ( 1.0 - balance ) ;
			if ( sideband[0] == 0 ) offset = 512-offset ;
			buf = &_sidetoneBuf[offset] ;
			...
			//  non-pano
			[ receiver[0] needSidetone:_sidetoneBuf inphase:inph quadrature:quad wide:wide samples:n wide:isWide[0] ] ;
			gain = monitorGain[0]*( ( isWide[0] ) ? 0.316 : ( 0.6/agcGain[0] ) ) ;			// -16 dB for wideband
			
			if ( mainChannels == 0x01 ) {
				// left channel
				for ( i = 0; i < n; i++ ) leftBuf[i] += gain*_sidetoneBuf[i] ;
			}
			else if ( mainChannels == 0x02 ) {
				// right channel
				for ( i = 0; i < n; i++ ) rightBuf[i] += gain*_sidetoneBuf[i] ;
			}
			else if ( mainChannels == 0x03 ) {
				// left and right channels
				for ( i = 0; i < n; i++ ) {
					v = gain*_sidetoneBuf[i] ;
					leftBuf[i] += v ;
					rightBuf[i] += v ;
				}
			}
	...
	if ( isEnabled[1] ) {
		if ( isWide[1] && isPano[1] ) {
			// pano (reverse for LSB)
			[ receiver[1] needSidetone:_panoBuf inphase:inph quadrature:quad wide:wide samples:n wide:YES ] ;
			CMPerformFIR( panoLow, _panoBuf, 512, _sidetoneBuf ) ;
			CMPerformFIR( panoHigh, _panoBuf, 512, _sidetoneBuf+512 ) ;
			...
			//  non-pano
			[ receiver[0] needSidetone:_sidetoneBuf inphase:inph quadrature:quad wide:wide samples:n wide:isWide[1] ] ;
...
//	for ( i = 0; i < n; i++ ) rightOutbuf[i] = CMSimpleFilter( rightRxFilter, rightBuf[i] ) ;
	...
	[ self addLeft:leftBuf right:rightOutbuf state:1 ] ;
}
...
- (void)retrieveForPlist:(Preferences*)pref
{
	...
	[ pref setInt:(int)[ [ mainChannel selectedItem ] tag ] forKey:kWBCWMainChannels ] ;
	[ pref setInt:(int)[ [ subChannel selectedItem ] tag ] forKey:kWBCWSubChannels ] ;
...
-(void) dealloc {
    NSLog(@"attempting to dealloc CWMonitor");
    free(_panoBuf);
    free(_sidetoneBuf);
    free(_localRightBuf);
    free(_localLeftBuf);
