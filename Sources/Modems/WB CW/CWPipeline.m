...
@synthesize  noiseBuffer = _noiseBuffer;
@synthesize dataBuffer   = _dataBuffer;
@synthesize iFiltered    = _iFiltered;
@synthesize qFiltered    = _qFiltered;
@synthesize iDecimateFiltered = _iDecimateFiltered;
@synthesize qDecimateFiltered = _qDecimateFiltered;
...
- (id)initFromClient:(CWMatchedFilter*)matchedFilter
{
	...
	if ( self ) {
        ...
        _noiseBuffer = malloc(2048 * sizeof(float));
        _dataBuffer  = malloc(2048 * sizeof(float));
        _iFiltered   = malloc(512 * sizeof(float));
        _qFiltered   = malloc(512 * sizeof(float));
        _iDecimateFiltered = malloc(512 * sizeof(float));
        _qDecimateFiltered = malloc(512 * sizeof(float));
        
		clearFloat( _dataBuffer, 2048 ) ;
		clearFloat( _noiseBuffer, 2048 ) ;
		dataBufferIndex = 0 ;
...
-(void) dealloc {
    NSLog(@"Attempting to dealloc CWPipeline");
    free(_noiseBuffer);
    free(_dataBuffer);
    free(_iFiltered);
    free(_qFiltered);
    free(_iDecimateFiltered);
    free(_qDecimateFiltered);
}
...
- (void)getNoiseThresholdFromHistogram
{
	...
	for ( j = 0; j < 100; j++ ) {
		k = ( dataBufferIndex + j - 101 + 2048 ) & 0x7ff ;
		count = _noiseBuffer[k]/peak*239.0 ;
...
- (void)importArray:(float*)array
{
	...
	CMPerformFIR( iAGCFilter, array, 512, _iFiltered ) ;
	CMPerformFIR( qAGCFilter, array+512, 512, _qFiltered ) ;
	CMPerformFIR( iDecimateFilter, array, 512, _iDecimateFiltered ) ;
	CMPerformFIR( qDecimateFilter, array+512, 512, _qDecimateFiltered ) ;
	...
	for ( i = 0; i < 64; i++ ) {
		...
		x = _iFiltered[j] ;
		y = _qFiltered[j] ;
		_noiseBuffer[dataBufferIndex] = sqrt( x*x + y*y ) ;
		x = _iDecimateFiltered[j] ;
		y = _qDecimateFiltered[j] ;
		_dataBuffer[dataBufferIndex] = sqrt( x*x + y*y ) ;
