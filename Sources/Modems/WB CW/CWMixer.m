...
{
    if(dda->freq <= 0.0) dda = &g_mark; //if the local dda hasn't been set - use the global;
	int t, mst, lst ;
	double th ;
	CMAnalyticPair p ;
...
- (void)setTonePair:(const CMTonePair*)tonepair
{
    ...
    [ self setDDA: &g_mark freq:tonepair->mark ];
}
...
- (void)importData:(CMPipe*)pipe
{
	...
	if ( isAural ) {
		...
		if ( _receiver ) [ _receiver received:iIF quadrature:qIF wide:array samples:512 ] ;
		return ;
	}
