...
- (id)initWithDemodulator:(CMFSKDemodulator*)demod
{
	...
	if ( self ) {	
		...
		name = [ NSString stringWithCString:"~/Library/Application Support/cocoaModem/Morse.txt" encoding:NSASCIIStringEncoding] ;
		...
		if ( ext ) {
			while ( 1 ) {
				index = 0 ;
				string[0] = 0 ;
				if ( (fscanf( ext, "%d %s", &index, string ) == 0) || (index <= 0) ) break ;
