...
OSStatus netsendRenderer( void* ref, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 frames, AudioBufferList *ioData )
{
	...
	ns = (__bridge NetAudioStruct*)ref ;
...
- (void)tick:(NSTimer*)timer
{
	...
	if ( self.netAudioStruct.runState == kNetAudioStopped || self.netAudioStruct.runState == kNetAudioIdle ) {
		[ timer invalidate ] ;
...
- (Boolean)setPassword:(NSString*)name
{
    ...
    if(name == nil)  password = [[NSString alloc]initWithString:name];
...
- (Boolean)setupNetSendUnit
{
	...
	status = AudioComponentInstanceNew( component, &netSendAudioUnit ) ;
...
- (Boolean)runThread
{
	...
	if ( self.netAudioStruct.delegate == nil || !netSendAudioUnit || self.runTimer != nil ) return NO ;
