...
//	serviceName = ( name != nil ) ? [ name retain ] : @"AUNetSend" ;
    if(name == nil) serviceName = @"AUNetSend";
...
- (Boolean)setAddress:(const char*)ip port:(int)port
{
	...
	if ( self.netAudioStruct.delegate && [ self.netAudioStruct.delegate respondsToSelector:@selector(netReceive:addressChanged:port:) ] ) [ self.netAudioStruct.delegate netReceive:self addressChanged:ip port:port  ] ;
	return YES ;
}
...
//	OSStatus status ;
	...
	if ( self.netAudioStruct.runState != kNetAudioRunning ) {
		[ timer invalidate ] ;
		return ;
	}
	if ( [ self.tickLock tryLock ] ) {
		actionFlags = 0 ;
		 AudioUnitRender( netReceiveAudioUnit, &actionFlags, &timeStamp, 0, samplesPerBuffer, &bufferList ) ;
		timeStamp.mSampleTime += samplesPerBuffer ;
		// send data to delegate
		if ( self.netAudioStruct.delegate && [ self.netAudioStruct.delegate respondsToSelector:@selector(netReceive:newSamples:left:right:) ] ) [ self.netAudioStruct.delegate netReceive:self newSamples:samplesPerBuffer left:dataBuffer[0] right:dataBuffer[1] ] ;
...
- (Boolean)runThread
{
	...
	if ( self.netAudioStruct.delegate == nil || !netReceiveAudioUnit || self.runTimer != nil ) return NO ;
	[self setRunTimer:[ NSTimer scheduledTimerWithTimeInterval:samplesPerBuffer/samplingRate target:self selector:@selector(tick:) userInfo:self repeats:YES ]] ;
	[self.netAudioStruct setRunState : kNetAudioRunning] ;
	runLoop = [ NSRunLoop currentRunLoop ] ;
	//  thread will stay running in the run loop until the timer is stopped in the tick routine
	while ( self.netAudioStruct.runState != kNetAudioIdle && [ runLoop runMode:NSDefaultRunLoopMode beforeDate:[ NSDate distantFuture ] ] ) ;
	//[ runTimer release ] ;
	[self setRunTimer:nil];
...
{
	[ self.timerThreadLock unlockWithCondition:kCommandAvailable ] ;
	return YES ;
}
...
- (void)stopSampling
{
    ...
    [self.netAudioStruct setRunState : kNetAudioIdle ];
}
...
- (Boolean)setupNetReceiveUnit:(Boolean)useBonjour
{
	...
	status = AudioComponentInstanceNew( component, &netReceiveAudioUnit ) ;
...
- (Boolean)setupWithService:(NSString*)service delegate:(id)inDelegate samplesPerBuffer:(int)size useBonjour:(Boolean)useBonjour
{
	...
	socket = nil ;
	netReceiveAudioUnit = nil ;
	[self.netAudioStruct setDelegate : inDelegate] ;
...
- (void)bonjourNetReceiveConnect:(BonjourSocket*)inSocket
{
	...
	if ( self.netAudioStruct.delegate && [ self.netAudioStruct.delegate respondsToSelector:@selector(netReceive:addressChanged:port:) ] ) [ self.netAudioStruct.delegate netReceive:self addressChanged:[ inSocket ip ] port:[ socket port ]  ] ;
}
...
{
	[ self stopSampling ] ;
	if ( self.netAudioStruct.delegate && [ self.netAudioStruct.delegate respondsToSelector:@selector(netReceive:disconnectedFromAddress:port:) ] ) [ self.netAudioStruct.delegate netReceive:self disconnectedFromAddress:[ inSocket ip ] port:[ socket port ] ] ;
}
